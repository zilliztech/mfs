...
幂等性不依赖顺序：`chunk_id = sha1(namespace_id + connector_uri + object_uri + chunk_kind + locator)` 跟处理顺序无关。priority 只影响体感和调度便利性，不影响正确性。
...
`locator` 保证 chunk_id 在 **同一 object 内唯一**，按 chunk_kind 取不同形态：

- 结构化对象（DB row / thread / issue）→ 连接器的 PK dict（`{"id":1}` / `{"thread_ts":"..."}` / `{"number":42}` ...，见 [06 §3](06-search-and-retrieval.md#3-locator-schema-per-connector)）
- `body` / `document` / `code` chunk → 框架预留的 `{"lines":[start, end]}` 形态（行号区间作为 per-chunk 身份）
- `vlm_description` / `directory_summary` / `schema_summary` 这类每个 object 至多一条 → `null`，反正只此一份不会撞
