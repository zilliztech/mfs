...
schema.add_field("locator",       DataType.JSON, nullable=True)              # per-chunk 身份（统一字段）：结构化对象用 PK dict；body/code/document 用 {"lines":[s,e]}；once-per-object 类用 null
...
| 字段 | 含义 |
|---|---|
| `chunk_id` | `sha1(namespace_id + connector_uri + object_uri + chunk_kind + locator)`，幂等写入。namespace_id 必须进 hash——否则两个 namespace 注册同名外部数据源会让 chunk_id 撞车。`locator` 是 per-chunk 身份字段：结构化对象（row/thread/issue）用 PK dict；`body`/code/document 用 `{"lines":[s,e]}`；once-per-object 类用 null（详见 [02 §7](02-architecture.md#7-一致性)）|
...
| `locator` | per-chunk 身份（统一字段，三种形态）：结构化对象 → 连接器 PK dict；body/code/document → `{"lines":[start,end]}`；once-per-object 类 → null。framework 预留 `lines` 为 reserved key（连接器 `locator_fields` 不能用） |
...
| `directory_summary` | 目录概览（递归，自底向上） | 一个目录的功能描述，由其文件内容 + 子目录摘要汇总 |
| `schema_summary` | 表/集合 schema 推导描述 | postgres 表 + 业务字段的语义描述 |

> 没有「单文件 LLM 摘要」这种 chunk_kind：整文件摘要只在**目录**粒度才有意义（见 §6 directory）。

新增 chunk_kind 要走框架升级流程，不通过 connector TOML 加。

search 默认全 kind 召回，用 `mfs search ... --kind body,directory_summary` 限定。

## 3. locator schema per connector

每个 connector 的 `locator` JSON schema 是稳定契约，agent 按这个 parse。统一字段，三种形态：

- 结构化 chunk（row / thread / issue）→ 连接器 PK dict
- body / code / document chunk → `{"lines":[start,end]}`
- once-per-object 类（dir/schema/vlm summary）→ `null`

| Connector | body/code/doc 形态 | structured 形态 |
|---|---|---|
| `file` | `{"lines":[start,end]}` | — |
| `github code` | `{"lines":[start,end]}` | — |
| `web` | `{"lines":[start,end]}`（页面级 markdown 切分） | — |
| `github issues / pulls` | — | `{"number": int}` |
| `gdrive` | — | `{"file_id": str, "revision_id": str}` |
| `s3 / r2 / gcs` | `{"lines":[start,end]}` | — |
...
|---|---|---|
| `document` (md/pdf/docx/gdoc/html→md) | `body` | 一个 markdown 段落 / 一段抽取文本 |
...
| `message_stream` (messages.jsonl) | `thread_aggregate` | 一个 thread / session 的对话拼接 |
| `record_collection` (issues.jsonl/records.jsonl) | `record_aggregate` 或 `row_text` | 一条 record 拼字段 |
| （目录，非枚举对象） | `summary.enabled` 时产 `directory_summary` | 见 §6.1 递归目录摘要 |

### 6.1 递归目录摘要

`directory_summary` 是**唯一**的 LLM 整体摘要 chunk（没有单文件摘要——对一个 `.py` 或一篇文档单独做整文件摘要意义不大，整体语义在目录这一层）。默认关闭（`[summary] enabled`），opt-in。

不靠 connector 枚举目录对象，而是文件索引阶段跑完后由 framework 起**第二阶段**：

1. 从本次 job 改动过的对象路径，推出受影响的**所有祖先目录**（含连接器根 `/`）。
2. 这些目录作为 `dir_summary` 任务**进同一条队列**，`priority = -depth`，所以共享的 worker 处理循环**最深的先跑**。任务串行执行 → 父目录一定在它所有子目录之后处理。
3. 每个目录的摘要输入 = 直属文件的内容摘录（md/代码原文、pdf/html/docx 取已转好的 `converted_md`、图片仅在 `include_image_desc` 时取 VLM 描述，各自截到 `per_file_max_kb`）+ 直属子目录**已经算好的** `directory_summary`（从 Milvus 读回），整体截到 `max_input_kb`。
4. 因此里层摘要自底向上**逐层上卷**进高层摘要，根目录摘要能反映整棵树。空目录（无可摘要内容）不产 chunk，并清掉旧摘要。

文件内容摘录复用第一阶段已写入的 artifact（`converted_md` / `vlm_text`），不重复转换、不重复调 VLM。增量更新时只有改动文件的祖先链会重算，其余目录摘要原样保留。
...
Milvus sparse search:
  ...
  返回带 chunk content 的 hits → chunk 片段 + locator（body chunk 形如 `{"lines":[s,e]}`，结构化对象是连接器 PK dict）
```
...
[summary]
enabled  = false                # 总开关，默认关；开启后才做递归目录摘要（见 §6.1）
provider = "openai"
model    = "gpt-4o-mini"
max_tokens = 800
dir_recursive    = true         # 自底向上递归（子目录摘要上卷进父目录）；false = 只摘连接器根
max_input_kb     = 64           # 喂给单个目录摘要的总输入上限（超出截断）
per_file_max_kb  = 16           # 单文件摘录上限，防一个大文件吃满预算
include_image_desc = false      # 是否把图片的 VLM 描述喂进目录摘要
...
- 换 summary / vlm → 只影响 `directory_summary` / `schema_summary` / `vlm_description` 这几种 chunk_kind 的行，body chunk 不动
...
| `directory` | 不进 Milvus | `ls / tree` 看结构；`summary.enabled` 时由递归目录摘要阶段产 `directory_summary`（见 §6.1） |
...
- **固定外壳**（每个 connector 都填同样的顶层 key）：`source / locator / content / score / metadata{kind, chunk_kind, connector_type, media_type, fields}`。agent 只认这层，跨 connector 一致
- **可变内核**：`locator` 的内部结构（postgres `{schema,table,pk}` / slack `{channel,date,thread_ts}` / body chunk `{"lines":[s,e]}`）和 `metadata.fields` 的字段（ticket 的 status/priority、PR 的 merged_at）per-connector。但每个 connector 的 locator schema 是**文档化的稳定契约**（见 [§3](#3-locator-schema-per-connector) 的表 + skill 里该 connector 的 reference 文档），不是自由发挥

agent 读外壳，遇到 locator 内核就查该 connector 文档化的 schema——**差异是"被文档化的差异"**。

### locator 三种形态

`locator` 是统一字段，三种形态由 chunk_kind 决定，agent 根据里面有什么 key 来选 reopen 命令：

| chunk_kind | locator 形态 | 重开方式 |
|---|---|---|
| `body`（document/code）| `{"lines": [start, end]}` | `cat --range start:end`（或 `cat --locator '{"lines":[s,e]}'`） |
| `row_text`（DB row / record）| 连接器 PK dict（`{"id": 12}` 等）| `cat --locator '{...}'` |
| `thread_aggregate`（slack/discord/gmail）| `{"thread_ts": "...", "chunk_index": 0, "msg_range":[s,e]}` | `cat --locator '{"thread_ts":"..."}'` |
| `record_aggregate`（issue+comments）| `{"number": int}` | `cat --locator '{"number":N}'` |
| `vlm_description`（图片）| `null`（用 source 本身）| `cat <source> --meta` |
| `directory_summary` / `schema_summary` | `null` | 跟随其来源 object |

框架预留 `lines` 作为 `locator` dict 的 reserved key——连接器的 `[[objects]].locator_fields` 不能声明 `lines`，启动校验会拒绝，避免跟 body chunk 的身份形态撞键。
