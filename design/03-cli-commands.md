...
- 否则对象已索引 → Milvus `sparse_vec` BM25：**统一、便宜、CS 友好**；关键词级、非 regex 精确，返回 chunk 片段（带 `locator`，body chunk 的 `locator` 形如 `{"lines":[s,e]}`，结构化对象的 `locator` 是 PK dict）
...
| `locator` | per-chunk 身份字段（统一），三种形态：body/code/document → `{"lines":[start,end]}`；结构化对象 → 连接器 PK dict；once-per-object → null。schema per connector（见 [06 §3](06-search-and-retrieval.md#3-locator-schema-per-connector)） |
