...
- [x] **protocol/OpenAPI + 多语言 SDK**：FastAPI 路由加 pydantic typed models（`api/models.py`，ResultEnvelope 等）→ 导出 `protocol/openapi.yaml`（OpenAPI 3.1，8 路径，operationId）+ `protocol/schemas/openapi.json` + `protocol/errors.md`。由 openapi-generator 7.22 生成（`sdks/generate.sh` 可复现），按 tag 分 Server/Ingest/Retrieval/Browse Api。当初 4 个 SDK 全对真实 server 端到端测过（python 10/10、typescript 9/9、go 9/9、java 9/9）；**后续收窄交付范围：只保留 `sdks/{python,typescript}` + CLI**，go/java 已移除（同一份 spec 随时可 `-g go/-g java` 再生成回来，零锁定）。冒烟工程在 `sdks/smoke/{python,typescript}`（不发布）。
