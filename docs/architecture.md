...
| Artifact cache | `server/python/src/mfs_server/storage/` | Stores derived blobs such as converted document text and image descriptions on the local filesystem. Mount a volume at the cache root to persist it. | [Configuration](configuration.md), [Deployment](deployment.md) |
