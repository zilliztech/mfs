# mfs-sdk
Multi-source File-like Search — HTTP /v1 control plane.

This Python package is automatically generated by the [OpenAPI Generator](https://openapi-generator.tech) project:

- API version: 0.4.4
- Package version: 0.4.4
- Generator version: 7.22.0
- Build package: org.openapitools.codegen.languages.PythonClientCodegen

> **Current MFS runtime guidance**
>
> This README is generated OpenAPI reference plus a checked-in runtime overlay.
> For curated v0.4 guidance, use [`docs/sdks.md`](../../docs/sdks.md) and
> [`docs/api.md`](../../docs/api.md).
>
> - Package metadata in [`pyproject.toml`](pyproject.toml) is `mfs_sdk`
>   `0.4.4`.
> - Normal `mfs-server run` / `mfs-server api` runs default to
>   `http://127.0.0.1:13619`.
> - The generated client default is `http://127.0.0.1:8765`; use that only for
>   generated or smoke-test runs when you intentionally start a server there.
> - `mfs-server run` / `mfs-server api` bootstrap or reuse
>   `$MFS_HOME/server.token` unless `auth_token = "-"` is configured.
> - When auth is enabled, every request except `GET /healthz` must include
>   `Authorization: Bearer <token>`.
> - Generated "no authorization" text comes from the OpenAPI scaffold, not from
>   the runtime auth middleware.

## Current Runtime Setup

Set the base URL and token explicitly before creating API classes:

```bash
export MFS_URL=http://127.0.0.1:13619
export MFS_TOKEN=replace-with-your-token
```

```python
import os

import mfs_sdk


configuration = mfs_sdk.Configuration(
    host=os.getenv("MFS_URL", "http://127.0.0.1:13619")
)

with mfs_sdk.ApiClient(configuration) as api_client:
    token = os.getenv("MFS_TOKEN")
    if token:
        api_client.set_default_header("Authorization", f"Bearer {token}")

    server = mfs_sdk.ServerApi(api_client)
    print(server.get_server_info().version)
```

## Requirements.

Python 3.10+

## Installation & Usage

This checked-in README does not assert package availability. Install or link
the checked-in `sdks/python` package through the repository release or
development workflow you are using, then import the package:

```python
import mfs_sdk
```

### Tests

Execute `pytest` to run the tests.

## Getting Started

Please follow the [installation procedure](#installation--usage) and then run the following:

```python

import os

import mfs_sdk
from mfs_sdk.rest import ApiException
from pprint import pprint

# For normal mfs-server run/api, use http://127.0.0.1:13619.
# The generated default http://127.0.0.1:8765 is for generated/smoke runs.
# See configuration.py for a list of all supported configuration parameters.
configuration = mfs_sdk.Configuration(
    host = os.getenv("MFS_URL", "http://127.0.0.1:13619")
)



# Enter a context with an instance of the API client
with mfs_sdk.ApiClient(configuration) as api_client:
    token = os.getenv("MFS_TOKEN")
    if token:
        api_client.set_default_header("Authorization", f"Bearer {token}")

    # Create an instance of the API class
    api_instance = mfs_sdk.BrowseApi(api_client)
    path = os.getenv("MFS_TARGET", "/tmp/mfs_sdk_fixture/auth.md") # str |
    read_range = None # str | for example "1:120" (optional)
    meta = False # bool |  (optional) (default to False)
    density = None # str | for example "normal" (optional)

    try:
        # Cat
        api_response = api_instance.cat(path, range=read_range, meta=meta, density=density)
        print("The response of BrowseApi->cat:\n")
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling BrowseApi->cat: %s\n" % e)

```

## Documentation for API Endpoints

All URIs are relative to the configured client host. The generated default host
is *http://127.0.0.1:8765*; normal `mfs-server run` / `mfs-server api` runs
default to *http://127.0.0.1:13619*.

Class | Method | HTTP request | Description
------------ | ------------- | ------------- | -------------
*BrowseApi* | [**cat**](docs/BrowseApi.md#cat) | **GET** /v1/cat | Cat
*BrowseApi* | [**export**](docs/BrowseApi.md#export) | **GET** /v1/export | Export
*BrowseApi* | [**head**](docs/BrowseApi.md#head) | **GET** /v1/head | Head
*BrowseApi* | [**ls**](docs/BrowseApi.md#ls) | **GET** /v1/ls | Ls
*BrowseApi* | [**tail**](docs/BrowseApi.md#tail) | **GET** /v1/tail | Tail
*ConnectorsApi* | [**estimate_connector**](docs/ConnectorsApi.md#estimate_connector) | **POST** /v1/connectors/estimate | Estimate
*ConnectorsApi* | [**inspect_connector**](docs/ConnectorsApi.md#inspect_connector) | **GET** /v1/connectors/inspect | Inspect
*ConnectorsApi* | [**probe_connector**](docs/ConnectorsApi.md#probe_connector) | **POST** /v1/connectors/probe | Probe
*ConnectorsApi* | [**remove_connector**](docs/ConnectorsApi.md#remove_connector) | **DELETE** /v1/connectors | Remove
*IngestApi* | [**add_source**](docs/IngestApi.md#add_source) | **POST** /v1/add | Add
*IngestApi* | [**cancel_job**](docs/IngestApi.md#cancel_job) | **POST** /v1/jobs/{job_id}/cancel | Cancel Job
*IngestApi* | [**files_manifest**](docs/IngestApi.md#files_manifest) | **POST** /v1/files/manifest | Files Manifest
*IngestApi* | [**files_upload**](docs/IngestApi.md#files_upload) | **PUT** /v1/files/upload | Files Upload
*IngestApi* | [**get_job**](docs/IngestApi.md#get_job) | **GET** /v1/jobs/{job_id} | Job
*IngestApi* | [**list_jobs**](docs/IngestApi.md#list_jobs) | **GET** /v1/jobs | List Jobs
*IngestApi* | [**upload_source**](docs/IngestApi.md#upload_source) | **POST** /v1/upload | Upload
*RetrievalApi* | [**grep**](docs/RetrievalApi.md#grep) | **GET** /v1/grep | Grep
*RetrievalApi* | [**search**](docs/RetrievalApi.md#search) | **GET** /v1/search | Search
*ServerApi* | [**get_server_info**](docs/ServerApi.md#get_server_info) | **GET** /v1/server/info | Server Info
*ServerApi* | [**healthz_healthz_get**](docs/ServerApi.md#healthz_healthz_get) | **GET** /healthz | Healthz
*ServerApi* | [**status**](docs/ServerApi.md#status) | **GET** /v1/status | Status


## Documentation For Models

Read the [generated reference notes](docs/README.md) before using generated
model examples. Placeholder JSON blocks in model pages are schema scaffolding,
not runnable runtime examples.

 - [AddRequest](docs/AddRequest.md)
 - [AddResponse](docs/AddResponse.md)
 - [CancelResponse](docs/CancelResponse.md)
 - [CatResponse](docs/CatResponse.md)
 - [ConnectorRow](docs/ConnectorRow.md)
 - [GrepMatchModel](docs/GrepMatchModel.md)
 - [GrepResponse](docs/GrepResponse.md)
 - [HTTPValidationError](docs/HTTPValidationError.md)
 - [JobResponse](docs/JobResponse.md)
 - [LocationInner](docs/LocationInner.md)
 - [LsEntry](docs/LsEntry.md)
 - [LsResponse](docs/LsResponse.md)
 - [ResultEnvelope](docs/ResultEnvelope.md)
 - [SearchResponse](docs/SearchResponse.md)
 - [ServerInfo](docs/ServerInfo.md)
 - [StatusResponse](docs/StatusResponse.md)
 - [ValidationError](docs/ValidationError.md)


<a id="documentation-for-authorization"></a>
## Documentation For Authorization

The OpenAPI spec declares bearer auth. If this checked-in generated client does
not attach authorization automatically, set the header explicitly:

```python
api_client.set_default_header("Authorization", f"Bearer {token}")
```

When the server is configured with `auth_token`, every request except
`GET /healthz` must include that header. See [`docs/api.md`](../../docs/api.md)
and [`docs/sdks.md`](../../docs/sdks.md).


## Author
