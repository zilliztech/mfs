...
def request_json(url: str, *, token: str | None = None, timeout: int = 20) -> tuple[bool, dict[str, Any]]:
    headers = {}
    if token:
...
def slack_api(method: str, token: str, params: dict[str, str] | None = None) -> tuple[bool, dict[str, Any]]:
...
def check_env() -> bool:
    required = ["SLACK_APP_TOKEN", "SLACK_BOT_TOKEN", "MFS_URL", "MFS_ALLOWED_SCOPES", "OPENTAG_BACKEND"]
    all_ok = True
    for name in required:
        ...
        if name == "SLACK_APP_TOKEN" and value:
            detail = "set, expected xapp-* token" if value.startswith("xapp-") else "set, unexpected prefix"
            ok = value.startswith("xapp-")
        if name == "SLACK_BOT_TOKEN" and value:
            detail = "set, expected xoxb-* token" if value.startswith("xoxb-") else "set, unexpected prefix"
            ok = value.startswith("xoxb-")
        print_check(ok, name, detail)
        all_ok = all_ok and ok

    mfs_token = token_from_env()
    print_check(bool(mfs_token), "MFS_TOKEN or ~/.mfs/server.token", "available" if mfs_token else "missing")
...
def check_mfs(scopes: list[str]) -> bool:
    ...
    print_check(ok, "MFS /v1/status", f"{connector_count} connectors" if ok else str(data.get("error")))
    all_ok = all_ok and ok
...
def check_slack(channel_id: str | None) -> bool:
    ...
    if channel_id:
        ...
        ok, data = slack_api("conversations.history", bot_token, {"channel": channel_id, "limit": "1"})
...
def check_backend() -> bool:
    ...
    if backend == "codex":
        ...
        print_check(ok, "backend codex", "codex executable found" if ok else "codex executable missing")
        return ok
    if backend == "claude":
        ok = shutil.which("claude") is not None
        print_check(ok, "backend claude", "claude executable found" if ok else "claude executable missing")
        return ok
    if backend == "custom":
        ok = bool(env("OPENTAG_BACKEND_COMMAND"))
        print_check(ok, "backend custom", "OPENTAG_BACKEND_COMMAND set" if ok else "missing command")
        return ok
    print_check(False, "OPENTAG_BACKEND", "must be codex, claude, or custom")
    return False
