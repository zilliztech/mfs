...
class GitHubPlugin(ConnectorPlugin):
    ...
    async def _paginated(self, client: httpx.AsyncClient, url: str) -> list[dict]:
        ...
        out, page = [], 1
        while len(out) < cap:
            resp = await client.get(url, params={"state": "all", "per_page": 100, "page": page}, timeout=30)
    ...
    def _flatten_issue(it: dict) -> dict:
        return {"number": it.get("number"), "title": it.get("title"), "body": it.get("body"),
                "state": it.get("state"), "user": (it.get("user") or {}).get("login"),
    ...
    def _flatten_pull(p: dict) -> dict:
        return {"number": p.get("number"), "title": p.get("title"), "body": p.get("body"),
    ...
    async def list(self, path: str) -> list[Entry]:
        if path in ("", "/"):
            ...
            if self._index_meta():
                top["_meta"] = "dir"
            return [Entry(n, t, self._media_type(n) if t == "file" else None) for n, t in sorted(top.items())]
    ...
    async def sync(self, opts: SyncOptions) -> AsyncIterator[ObjectChange]:
        ...
        async with httpx.AsyncClient(headers=self._headers()) as c:
            ...
            if self._index_meta():
                ...
                for p in pulls:
                    yield ObjectChange(uri=f"/_meta/pulls/{p['number']}/diff.patch", kind="modified")
