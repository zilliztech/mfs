...
async def main():
    ...
    base = f"/tmp/mfs_browse_{os.getpid()}"
    ...
    try:
        ...
        kw_hits = await eng.search("photosynthesis", connector_uri=uri, mode="keyword", top_k=5)
        sem_hits = await eng.search("photosynthesis", connector_uri=uri, mode="semantic", top_k=5)
        hy_hits = await eng.search("photosynthesis", connector_uri=uri, mode="hybrid", top_k=5)
        ...
        check(
            "keyword hit's content contains the literal term",
        ...
        src_hits = await eng.search(
            "retry decorator",
            connector_uri=uri,
            object_prefix=uri + "/src",
            mode="hybrid",
            top_k=10,
        )
        check(
            f"object_prefix='/src' restricts hits to src/ "
        ...
        body_only = await eng.search(
            "authenticate SAML", connector_uri=uri, chunk_kinds=["body"], mode="hybrid", top_k=10
        )
        # chunk_kind comes back nested under metadata in the envelope shape
        check(
            f"chunk_kinds=['body'] gates by chunk_kind ({len(body_only)} hits)",
            len(body_only) >= 1
            and all((h.get("metadata") or {}).get("chunk_kind") == "body" for h in body_only),
        )

        # chunk_kinds=['directory_summary'] returns 0 — summary is disabled
        no_dir = await eng.search(
            "photosynthesis",
            connector_uri=uri,
            chunk_kinds=["directory_summary"],
            mode="hybrid",
            top_k=10,
        )
        check(
            f"chunk_kinds=['directory_summary'] is empty when summary disabled "
            f"({len(no_dir)} hits)",
            len(no_dir) == 0,
        )

        # collapse=True: at most 1 hit per source (dedup by object)
        collapsed = await eng.search(
            "verify SAML SSO", connector_uri=uri, mode="hybrid", top_k=10, collapse=True
        )
        col_sources = [h.get("source") for h in collapsed]
        check(
            f"collapse=True yields one hit per source "
        ...
        check(
            f"ls(/src) lists code files (got {sorted(src_names)})",
        ...
        check(
            "cat(file) returns full text (contains 'verify_saml_sso_assertion')",
        ...
        check(
            f"cat(range=(2,6)) returns a contiguous 4-line slice ({len(rng_lines)} lines)",
            len(rng_lines) == 4
        ...
        check(
            f"cat(meta=True) returns a metadata dict (got keys={sorted(meta) if isinstance(meta, dict) else None})",
        ...
        check(
            f"cat(density='peek') on .py returns only code-symbol lines (lines={peek_lines})",
            peek_lines
            and all(
                any(ln.lstrip().startswith(tok) for tok in ("def ", "class ")) for ln in peek_lines
            ),
        )
        ...
        check(
            f"cat(density='peek') on .md returns only heading lines ({md_lines})",
        ...
        check("cat(density='skim') on .md is >= peek", len(skim_md) >= len(peek_md))
        ...
        check(
            f"grep literal: anchors a single hit in auth.py "
            f"({len(lit_hits)} hits, sources={sources_lit})",
            len(lit_hits) >= 1 and all((s or "").endswith("/src/auth.py") for s in sources_lit),
        )
        ...
        check(
            f"grep regex 'def \\w+' matches both code files (sources={re_sources})",
        ...
        check(
            f"grep path-scoped to /docs returns 0 'def' hits ({len(docs_hits)} hits)",
            len(docs_hits) == 0,
        )
        # top_k cap
