...
async def main():
    ...
    try:
        ...
        pending = await eng.meta.fetchall(
            "SELECT count(*) AS n FROM object_tasks WHERE connector_job_id=? AND status='pending'", (job_id,))
        ...
        jrow = await eng.meta.fetchone("SELECT status, succeeded_objects FROM connector_jobs WHERE id=?", (job_id,))
        check("A: job 'succeeded' after worker", jrow["status"] == "succeeded" and jrow["succeeded_objects"] == 3)
        ...
        repo2 = base + "_repo2"; os.makedirs(repo2)
        for i in range(2):
        ...
        ctasks = await eng.meta.fetchall(
            "SELECT count(*) AS n FROM object_tasks WHERE connector_job_id=? AND status='cancelled'", (job2,))
        ...
        cid2 = (await eng.meta.fetchone(
            "SELECT id FROM connectors WHERE root_uri LIKE ?", ("%" + os.path.basename(repo2),)))["id"]
