...
_MSGS = (
    [{"thread_id": "t-short", "ts": i, "text": f"short thread msg {i}"} for i in range(3)] +
    [{"thread_id": "t-long", "ts": i,
      "text": f"long thread message number {i} discussing {topic}"}
     for i, topic in enumerate([
        "sso login", "sso login", "sso login", "sso login", "sso login",
        "footer typo", "footer typo", "footer typo", "footer typo", "footer typo",
        "dark mode toggle", "dark mode toggle", "dark mode toggle", "dark mode toggle", "dark mode toggle",
        "payment gateway timeout", "payment gateway timeout", "payment gateway timeout",
        "payment gateway timeout", "payment gateway timeout",
        "kubernetes deployment rollback", "kubernetes deployment rollback",
        "kubernetes deployment rollback", "kubernetes deployment rollback",
        "kubernetes deployment rollback",
        "high-contrast accessibility palette", "high-contrast accessibility palette",
        "high-contrast accessibility palette", "high-contrast accessibility palette",
...
class MemMessagesPlugin(ConnectorPlugin):
    NAME = "memmsgs"; URI_SCHEME = "memmsgs"; DISPLAY_NAME = "mem messages (test)"; PROMPT = "t"
...
async def main():
    ...
    base = f"/tmp/mfs_thr_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    eng._resolve_target = lambda t: ("memmsgs", t, "memmsgs", {}) if t.startswith("memmsgs://") else orig(t)
    await eng.startup()
    conn_uri = "memmsgs://t"
    obj_uri = "/messages.jsonl"
    try:
        eng.milvus.drop_collection("default"); eng.milvus.ensure_collection("default")
        cfg_obj = {"objects": [{"match": "*messages.jsonl",
                                "text_fields": ["text"],
                                "group_by": "thread_id",
        ...
        check(f"thread aggregation produced multi-chunk output (got {o['chunk_count']} chunks, want >=3)",
        ...
        check(f"long thread (t-long) -> multiple sub-chunks (got {len(long_chunks)})",
              len(long_chunks) >= 2)
        ...
        check("long-thread sub-chunks have contiguous chunk_index starting at 0",
              idxs == list(range(len(long_chunks))))
        has_ranges = all(
            isinstance((c.get("locator") or {}).get("msg_range"), list) and
            len((c.get("locator") or {}).get("msg_range") or []) == 2
        ...
        res = await eng.search("graceful shutdown signal handling",
                               connector_uri=conn_uri, mode="hybrid", top_k=5)
        late_hit = [r for r in res
                    if (r.get("locator") or {}).get("thread_id") == "t-long"
                    and "graceful shutdown" in (r.get("content") or "")]
        check("search finds the late-thread 'graceful shutdown' content "
