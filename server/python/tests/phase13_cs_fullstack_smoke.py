...
async def main():
    ...
    base = f"/tmp/mfs_cs_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg.object_store.backend = "s3"; cfg.object_store.bucket = "mfs-test"; cfg.object_store.prefix = ns
    cfg.object_store.endpoint_url = "http://127.0.0.1:9100"; cfg.object_store.region = "us-east-1"
    cfg.object_store.access_key_id = "mfsadmin"; cfg.object_store.secret_access_key = "mfsadmin123"
    ...
    try:
        ...
        art = await asyncio.to_thread(eng.object_store.get_artifact, ns,
                                      conn_uri + "/docs/guide.html", "converted_md")
        check("S3: converted_md artifact retrievable", art is not None and b"Deploy" in art)

        res = await eng.search("worker replicas behind the api", connector_uri=conn_uri, mode="hybrid", top_k=5)
