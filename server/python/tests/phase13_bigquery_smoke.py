...
def _up():
    ...
    try: return s.connect_ex(("127.0.0.1", 9050)) == 0
    finally: s.close()
...
def _bq_client():
    ...
    return bigquery.Client(project=PROJECT, client_options=ClientOptions(api_endpoint=EP),
...
async def main():
    ...
    if not _up():
        print("bigquery emulator not reachable on :9050 — start mfs-bq"); raise SystemExit(2)
    ...
    schema = [bq.SchemaField("id", "INT64"), bq.SchemaField("subject", "STRING"), bq.SchemaField("body", "STRING")]
    await asyncio.to_thread(lambda: c.create_table(bq.Table(ref, schema=schema)))
    errs = await asyncio.to_thread(lambda: c.insert_rows_json(ref, [
        {"id": 1, "subject": "SSO login fails", "body": "Users bounced to the identity provider after login"},
        {"id": 2, "subject": "Dark mode toggle", "body": "Add a dark theme switch to the settings page"},
        {"id": 3, "subject": "CSV export truncated", "body": "Large report export is cut at 65000 rows"}]))
    check("seeded bigquery table via job-free API", errs == [])

    base = f"/tmp/mfs_bq_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        sc = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + f"/{DS}/tables/{tbl}/schema.json")
        check("bigquery schema_summary produced", any(x.get("chunk_kind") == "schema_summary" for x in sc))

        res = await eng.search("single sign-on identity provider login", connector_uri=conn_uri, mode="hybrid", top_k=5)
        hit = next((e for e in res if e.get("locator")), None)
        check("bigquery search hit carries {id} locator", hit and "id" in (hit.get("locator") or {}))
        catout = await eng.cat(src, locator={"id": 1})
        recd = _json.loads(catout["content"])
        check("bigquery cat --locator exact record", recd.get("id") == 1 and "identity provider" in (recd.get("body") or ""))
    finally:
        try: eng.milvus.drop_collection("default")
        except Exception: pass
        await eng.shutdown()
        try: await asyncio.to_thread(lambda: c.delete_table(ref, not_found_ok=True))
        except Exception: pass
        c.close(); os.system(f"rm -rf '{base}'*")
