...
async def main():
    ...
    base = f"/tmp/mfs_fs_docs_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        if doc_objs:
            d = doc_objs[0]
            check(f"doc indexed with >=1 chunk (got {d['chunk_count']}, status={d['search_status']!r})",
                  (d["chunk_count"] or 0) >= 1)

            # 4) cat returns the doc body
            full_uri = conn_uri + d["object_uri"]
            content = await eng.cat(full_uri)
            check(f"cat returns the markdown body ({len(content) if isinstance(content, str) else '?'} chars)",
                  isinstance(content, str) and len(content) > 100)
            check("cat content includes a known phrase from the doc",
            ...
            check(f"search returns at least 1 hit on this feishu connector "
