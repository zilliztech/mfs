...
async def main():
    ...
    try:
        ...
        check(
            f"--all search returns hits from both connectors (A:{len(hitsA)} B:{len(hitsB)})",
            len(hitsA) >= 1 and len(hitsB) >= 1,
        )

        # ---- 2) search --connector-uri scopes to one ----
        resA = await eng.search(
            "authentication problems", connector_uri=uriA, mode="hybrid", top_k=10
        )
        only_A = all((r.get("source") or "").startswith(uriA) for r in resA)
        check(
            f"--connector-uri scopes to one (all {len(resA)} hits under {uriA})",
            only_A and len(resA) >= 1,
        )
        ...
        schema_hits = await eng.search(
            "payment", mode="hybrid", top_k=10, chunk_kinds=["schema_summary"]
        )
        check(f"--kind=body returns hits ({len(body_hits)})", len(body_hits) >= 1)
        check(
            f"--kind=schema_summary returns 0 (no DB connector) ({len(schema_hits)})",
            len(schema_hits) == 0,
        )
        ...
        check(
            "after .mfsignore + full=True: auth/login.md still indexed",
            any(p.endswith("auth/login.md") for p in paths),
        )
        check(
            "after .mfsignore + full=True: ops/incident-042.md pruned",
        ...
        check(
            f"chunks for pruned object are gone from Milvus "
            f"(get_chunks_by_object returns {len(chunks)})",
            len(chunks) == 0,
        )
        ...
        check(
            f"after remove: connector row gone (id rows={post_remove['n']})",
        ...
        check(
            f"after remove: search returns 0 hits from removed connector ({len(on_b)})",
            len(on_b) == 0,
        )
        # re-add same path
        await eng.add(str(repoB))
        ...
        check(f"after re-add: search returns hits from repoB again ({len(on_b2)})", len(on_b2) >= 1)
    finally:
        try:
            eng.milvus.drop_collection("default")
        except Exception:
