...
async def main():
    ...
    base = f"/tmp/mfs_discord_{os.getpid()}"
    ...
    try:
        ...
        check(
            f"channel has >=1 chunk (non-empty: {len(non_empty)} / {len(chan_objs)})",
            len(non_empty) >= 1,
        )
        ...
        if non_empty:
            ...
            check(
                f"cat --range 0:4 returns text ({len(cat_out) if isinstance(cat_out, str) else '?'} chars)",
