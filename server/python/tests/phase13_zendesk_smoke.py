...
async def main():
    ...
    if not os.environ.get("OPENAI_API_KEY") or not sub or not tok:
        print("need OPENAI_API_KEY + ZENDESK_SUBDOMAIN + ZENDESK_API_TOEKN"); raise SystemExit(2)
    base_url = f"https://{sub}.zendesk.com"
    auth = (f"{EMAIL}/token", tok)
    seeded = [
        {"subject": "MFSTEST payment capture fails", "body": "Stripe webhook returns 402 on checkout intermittently"},
        {"subject": "MFSTEST sso login loop", "body": "After SAML SSO the session token is not persisted"},
        {"subject": "MFSTEST csv export truncated", "body": "Large report export is cut at 65000 rows"}]
    created_ids = []
    ...
    basep = f"/tmp/mfs_zd_{os.getpid()}"; os.system(f"rm -rf '{basep}'*")
    cfg = load_server_config(apply_env=False)
    cfg.metadata.path = basep + "_m.db"; cfg.milvus.uri = basep + "_v.db"; cfg.milvus.token = ""
    cfg.object_store.root = basep + "_c"; cfg.transformation_cache.db_path = basep + "_t.db"
    ...
    try:
        ...
        res = await eng.search("stripe webhook 402 payment capture", connector_uri=conn_uri, mode="hybrid", top_k=5)
        check("zendesk tickets searchable", any("payment capture" in (e.get("content") or "").lower() for e in res)
              or len(res) > 0)
        hit = next((e for e in res if e.get("locator")), None)
        check("zendesk search hit carries a locator", hit is not None and isinstance(hit.get("locator"), dict))
        ...
        if hit:
            ...
            check("zendesk cat --locator returns a ticket record", isinstance(recd, dict) and "subject" in recd)
    finally:
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            for tid in created_ids:
                try: await c.delete(f"{base_url}/api/v2/tickets/{tid}.json", auth=auth)
                except Exception: pass
