...
async def _has_dirsum(eng, conn_uri, rel):
    rows = await asyncio.to_thread(
        eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + rel
    )
...
async def main():
    ...
    os.makedirs(f"{root}/bins", exist_ok=True)
    ...
    try:
        ...
        await eng.add(root)
        check(
            "R4.1 binary-only /bins has NO directory_summary",
            not await _has_dirsum(eng, conn_uri, "/bins"),
        )
        ...
        check(
            "R4.6 emptied /docs -> directory_summary purged",
            not await _has_dirsum(eng, conn_uri, "/docs"),
        )
    finally:
        try:
            eng.milvus.drop_collection("default")
    ...
    deep = root2 + "/a/b/c/d/e"
    os.makedirs(deep, exist_ok=True)
    open(f"{deep}/leaf.md", "w").write(
        "# Leaf\n\n" + ("payment idempotency key handling. " * 400)
    )  # big
    base2 = f"/tmp/mfs_sed_{os.getpid()}"
    os.system(f"rm -rf '{base2}'*")
    cfg2 = _mkcfg(base2)
    cfg2.summary.per_file_max_kb = 1
    cfg2.summary.max_input_kb = 4
    eng2 = Engine(cfg2)
    await eng2.startup()
    ...
    try:
        ...
        n_dir = await asyncio.to_thread(
            eng2.milvus.count, "default", 'chunk_kind == "directory_summary"'
        )
        ...
        check(
            "R4.5 summary input respected max_input_kb budget",
    ...
    base3 = f"/tmp/mfs_mt_{os.getpid()}"
    os.system(f"rm -rf '{base3}'*")
    cfg3 = _mkcfg(base3)
    cfg3.summary.enabled = False
    eng3 = Engine(cfg3)
    await eng3.startup()
    try:
        eng3.milvus.drop_collection("default")
        eng3.milvus.ensure_collection("default")
