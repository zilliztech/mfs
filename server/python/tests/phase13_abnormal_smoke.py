...
async def expect_raise(name, coro, *, clean_types=(ValueError, FileNotFoundError, KeyError, IsADirectoryError)):
    try:
        await coro
...
async def main():
    ...
    base = f"/tmp/mfs_abn_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg.auth_token = "tok"; cfg.summary.enabled = False
    eng = Engine(cfg)
    await eng.startup()
    ...
    try:
        ...
        await expect_raise("cat --locator on a document -> clean error", eng.cat(f"{root}/a.md", locator={"id": 1}))
        # 3. grep a path outside any connector
        await expect_raise("grep outside any connector -> clean error", eng.grep("x", "/no/such/place"))
        ...
        r = await eng.search("anything", connector_uri="file://local/nowhere", mode="hybrid", top_k=5)
        ...
        check("cat --range start>end -> empty, no crash", isinstance(out, str) and out.strip() == "")
        # 8. add postgres with a missing credential_ref -> clear error
        await expect_raise("add with missing env credential_ref -> clear error",
                           eng.add("postgres://bad", config={"credential_ref": "env:MFS_NOPE_MISSING", "schemas": ["public"]}))

        # 9. HTTP: job + auth abnormal paths
        app = create_app(cfg)
        with TestClient(app) as client:
            h = {"Authorization": "Bearer tok"}
            check("GET /v1/jobs/<bad> -> 404", client.get("/v1/jobs/deadbeef", headers=h).status_code == 404)
            cr = client.post("/v1/jobs/deadbeef/cancel", headers=h)
            check("cancel /v1/jobs/<bad> -> no 5xx, not cancelled",
                  cr.status_code < 500 and (cr.status_code >= 400 or cr.json().get("cancelled") is False))
            check("GET /v1/cat missing path param -> 422", client.get("/v1/cat", headers=h).status_code == 422)
            check("GET /v1/search missing q -> 422", client.get("/v1/search", headers=h).status_code == 422)
    finally:
