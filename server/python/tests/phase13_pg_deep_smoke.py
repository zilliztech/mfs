...
async def main():
    ...
    try:
        ...
        print(f"postgres not reachable: {e}"); raise SystemExit(2)
    # dedicated schema so the connector indexes ONLY this table (public has unrelated tables)
    sch = f"mfsdeep_{os.getpid()}"; tbl = "tickets"
    ...
    await conn.executemany(
        ...
        [(1, "SSO login fails", "Users bounced back to the identity provider after login", '{"sev":1}'),
         (2, "Dark mode toggle", "Add a dark theme switch to the settings page", '{"sev":3}'),
         (3, "Null body row", None, None)])   # NULL body + NULL jsonb

    base = f"/tmp/mfs_pgd_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    os.environ["MFS_TEST_PG_DSN"] = DSN     # via credential_ref so reopen (cat/re-add) works after redaction
    cfg_obj = {"credential_ref": "env:MFS_TEST_PG_DSN", "schemas": [sch], "objects": [{
        "match": "*rows.jsonl", "text_fields": ["subject", "body"],
        "metadata_fields": ["meta"], "locator_fields": ["id"], "chunk_strategy": "per_row"}]}
    try:
        ...
        sc = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + f"/{sch}/{tbl}/schema.json")
        check("schema_summary chunk produced", any(c.get("chunk_kind") == "schema_summary" for c in sc))

        # search + locator
        res = await eng.search("single sign-on bounced to identity provider",
                               connector_uri=conn_uri, mode="hybrid", top_k=5)
        hit = next((e for e in res if e.get("locator")), None)
        check("search hit carries {id} locator", hit and isinstance(hit["locator"], dict) and "id" in hit["locator"])
        ...
        check("cat --locator returns exact record", recd.get("id") == 1 and "identity provider" in (recd.get("body") or ""))
        try:
            await eng.cat(src, locator={"id": 999})
            check("missing locator raises", False)
        except Exception as ex:
            check("missing locator raises", "not_found" in str(ex).lower() or "locator" in str(ex).lower())
        ...
        check("NULL-column row still indexed/searchable", any((e.get("locator") or {}).get("id") == 3 for e in rn))

        # incremental MODIFY: update a row's body + cursor, re-add (no full) -> detected, reflected
        await conn.execute(f'UPDATE "{sch}"."{tbl}" SET body=$1, updated_at=now() WHERE id=2',
        ...
        check("incremental modify detected (table re-yielded)", any(t["change_kind"] == "modified" for t in tk))
        rm = await eng.search("high-contrast accessibility palette", connector_uri=conn_uri, mode="hybrid", top_k=5)
        check("modify reflected in search", any((e.get("locator") or {}).get("id") == 2 for e in rm))
        ...
        rd = await eng.search("single sign-on bounced to identity provider",
                             connector_uri=conn_uri, mode="hybrid", top_k=10)
        check("deleted row no longer retrievable", not any((e.get("locator") or {}).get("id") == 1 for e in rd))
    finally:
        try: eng.milvus.drop_collection("default")
        except Exception: pass
        await eng.shutdown()
        await conn.execute(f'DROP SCHEMA IF EXISTS "{sch}" CASCADE'); await conn.close()
