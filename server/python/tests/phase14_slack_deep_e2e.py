...
async def main():
    ...
    base = f"/tmp/mfs_skdeep_{os.getpid()}"
    ...
    try:
        ...
        users_obj = await eng.meta.fetchone(
            "SELECT chunk_count FROM objects WHERE connector_id=? AND object_uri='/users.jsonl'",
            (cid,),
        )
        print(
            f"  DEBUG channels={len(chan_objs)} top non-empty:"
            f" {[(c['object_uri'].split('/')[2], c['chunk_count']) for c in chan_objs[:3]]}"
        )
        check(f"T1 >=1 public channel enumerated ({len(chan_objs)})", len(chan_objs) >= 1)
        non_empty = [r for r in chan_objs if (r["chunk_count"] or 0) > 0]
        check(
            f"T1 at least one channel has thread_aggregate chunks "
        ...
        users_status = await eng.meta.fetchone(
            "SELECT 1 FROM objects WHERE connector_id=? AND object_uri='/users.jsonl'", (cid,)
        )
        ...
        check(
            "T2 finding: PROMPT advertises /users.jsonl but sync() never "
        ...
        check(
            f"T3 ls /channels entries follow <name>__<id> shape ({len(dir_names)} entries)",
        ...
        sample_chunks = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + sample_chan_path
        )
        import re as _re
        ...
        for c in sample_chunks:
            ...
            tokens += [
                ...
                if w.lower() not in {"slack", "channel", "message", "thread", "request"}
            ]
        # pick a mid-list token to bias away from boilerplate
        if tokens:
            term = tokens[len(tokens) // 2]
            res = await eng.search(term, connector_uri=conn_uri, mode="hybrid", top_k=10)
            on_chan = [r for r in res if "/channels/" in (r.get("source") or "")]
            check(
                f"T4 search('{term}') surfaces hits under /channels/ "
                f"({len(res)} total, {len(on_chan)} on channels)",
                len(on_chan) >= 1,
            )
        else:
            check("T4 sample channel too thin for unique-term probe (skipped)", True)
        ...
        check(
            f"T5 every hit is chunk_kind='thread_aggregate' ({len(agg_only)} hits)",
            len(agg_only) == 0
            or all(
                (h.get("metadata") or {}).get("chunk_kind") == "thread_aggregate" for h in agg_only
            ),
        )
        ...
        check(
            f"T6 scoped hits never come from /users.jsonl ({len(scoped)} hits)",
            len(scoped) == 0
            or all(
        ...
        sample_msg = next(
            (
                c
                for c in sample_chunks
                if isinstance(c.get("locator"), dict) and "thread_ts" in c["locator"]
            ),
            None,
        )
        if sample_msg is None:
            sample_msg = next(
                (c for c in sample_chunks if isinstance(c.get("locator"), dict)), None
            )
        if sample_msg:
            loc = sample_msg["locator"]
            cat_res = await eng.cat(conn_uri + sample_chan_path, locator=loc)
            check(
                f"T7 cat --locator reopens a message ({loc})",
            ...
            check(
                "T7 no message carries a structured locator in sample chunks "
                "(workspace-content-dependent; skipped)",
                True,
            )
        ...
        check(
            f"T8 finding: slack re-enqueues every channel each sync "
            f"(channel-grained fingerprint; got {delta_tasks} new tasks)",
            delta_tasks >= len(chan_objs) - 1,
        )
        check(
            f"T8 second sync embeds 0 chunks — tx_cache absorbs the cost "
