...
async def sync_uris(plugin):
    return [(c.uri, c.kind) async for c in plugin.sync.__wrapped__(plugin)] if hasattr(plugin.sync, "__wrapped__") \
...
async def main():
    ...
    check("bq object_kind rows.jsonl -> table_rows",
          bq.object_kind_of("/ds/tables/t/rows.jsonl") == "table_rows")
    check("bq object_kind schema.json -> table_schema",
    ...
    check("bq list table -> schema.json+rows.jsonl",
    ...
    check("snowflake object_kind 5-part rows.jsonl",
    ...
    check("jira object_kind issues.jsonl -> record_collection",
          jira.object_kind_of("/projects/ENG/issues.jsonl") == "record_collection")
    flat = jira._flatten_issue({"key": "ENG-1", "id": "10",
        "fields": {"summary": "Login bug", "description": "broken",
                   "status": {"name": "Open"}, "priority": {"name": "High"},
                   "assignee": {"displayName": "Ann"}, "labels": ["auth"]}})
    check("jira flatten extracts summary/status/assignee",
    ...
    check("linear auth header is raw key (not Bearer)",
          lin._headers()["Authorization"] == "k")
    fl = LinearPlugin._flatten({"identifier": "ENG-2", "title": "T", "description": "d",
        "state": {"name": "Todo"}, "assignee": {"name": "Bo"},
        "labels": {"nodes": [{"name": "bug"}]}})
    check("linear flatten state/assignee/labels",
    ...
    check("notion heading_1 -> '# '", _block_to_md(
        {"type": "heading_1", "heading_1": {"rich_text": [{"plain_text": "Title"}]}}) == "# Title")
    check("notion to_do checked -> [x]", _block_to_md(
        {"type": "to_do", "to_do": {"checked": True, "rich_text": [{"plain_text": "done"}]}}) == "- [x] done")
    ...
    check("zendesk object_kind records.jsonl", zd.object_kind_of("/tickets/records.jsonl") == "record_collection")
    check("zendesk comments.jsonl -> record_collection", zd.object_kind_of("/tickets/comments.jsonl") == "record_collection")
    ...
    zlist = await zd.list("/")
    check("zendesk root -> tickets/users/organizations",
          sorted(e.name for e in zlist) == ["organizations", "tickets", "users"])
    zt = await zd.list("/tickets")
    check("zendesk /tickets -> records.jsonl+comments.jsonl",
    ...
    check("salesforce object_kind records.jsonl", sfc.object_kind_of("/Account/records.jsonl") == "record_collection")
    sflist = await sfc.list("/")
    check("salesforce root lists configured objects", [e.name for e in sflist] == ["Account", "Case"])
    ...
    check("hubspot object_kind records.jsonl", hs.object_kind_of("/contacts/records.jsonl") == "record_collection")

    # ---------------- slack ----------------
    from mfs_server.connectors.slack.plugin import SlackPlugin, _sanitize
    sl = make(SlackPlugin, {"token": "xoxb"})
    check("slack object_kind messages.jsonl -> message_stream",
    ...
    check("discord object_kind messages.jsonl -> message_stream",
    ...
    check("gdrive native doc export suffix .txt -> document",
          gd.object_kind_of("/Folder/Doc.txt") == "document")
    check("gdrive native types map present",
          "application/vnd.google-apps.document" in _NATIVE)
    await gd.state.set("files", {"/Team/Spec.txt": {"id": "1", "mimeType": "x", "fingerprint": "f"},
    ...
    check("feishu object_kind messages.jsonl -> message_stream",
          fs.object_kind_of("/chats/team__oc1/messages.jsonl") == "message_stream")
    check("feishu _extract_text text type",
          _extract_text("text", '{"text":"hi there"}') == "hi there")
    check("feishu _extract_text post type",
