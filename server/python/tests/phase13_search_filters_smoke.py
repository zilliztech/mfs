...
async def main():
    ...
    os.makedirs(f"{root}/my_dir", exist_ok=True)
    os.makedirs(f"{root}/myXdir", exist_ok=True)
    ...
    open(f"{root}/myXdir/skip.md", "w").write(
        "# Other\n\nStorage and caching in the sibling directory.\n"
    )
    base = f"/tmp/mfs_filt_{os.getpid()}"
    ...
    try:
        ...
        check(
            "C6 collapse: keep.md split into >1 body chunk pre-collapse",
        ...
        r = await eng.search(
            "storage caching",
            connector_uri=sc_uri,
            object_prefix=sc_prefix,
            mode="hybrid",
            top_k=10,
        )
        ...
        with TestClient(app) as client:
            ...
            check(
                "HTTP ?kind=directory_summary filters",
                hr.status_code == 200 and hk == {"directory_summary"},
            )
    finally:
        try:
            eng.milvus.drop_collection("default")
