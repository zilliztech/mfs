...
async def main():
    if not (os.environ.get("OPENAI_API_KEY") and os.environ.get("GITHUB_TOKEN")):
        print("need OPENAI_API_KEY + GITHUB_TOKEN — run via bash -ic"); raise SystemExit(2)
    base = f"/tmp/mfs_ghm_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        gh_config = {
            "repo": "octocat/Hello-World", "index_meta": True, "max_read_rows": 3,
            "objects": [
                {"match": "*issues.jsonl", "text_fields": ["title", "body"],
                 "locator_fields": ["number"], "chunk_strategy": "per_row", "chunk_max": 3},
                {"match": "*pulls.jsonl", "text_fields": ["title", "body"],
                 "locator_fields": ["number"], "chunk_strategy": "per_row", "chunk_max": 3},
            ],
        }
        ...
        diffs = await eng.meta.fetchall(
            "SELECT object_uri, chunk_count FROM objects WHERE connector_id=? AND object_uri LIKE '/_meta/pulls/%/diff.patch'", (cid,))
        check("at least one PR diff.patch document indexed",
        ...
        res = await eng.search("hello world", connector_uri="github://hello",
                               object_prefix="github://hello/_meta/issues.jsonl", mode="hybrid", top_k=3)
        check("issues searchable + carry number locator",
