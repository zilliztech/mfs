...
async def main():
    ...
    base = f"/tmp/mfs_jrdeep_{os.getpid()}"
    ...
    try:
        ...
        check(
            f"T1 {issues_uri} indexed (chunks={issue_ro.get('chunk_count')})",
            issue_ro.get("chunk_count") is not None and issue_ro["chunk_count"] >= 1,
        )
        check(
            f"T1 max_read_rows=15 caps the chunk_count (got {issue_ro.get('chunk_count')})",
        ...
        chunks = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + issues_uri
        )
        keys = [(c.get("locator") or {}).get("key") for c in chunks]
        check(
            f"T2 every chunk carries a {{'key': 'XXX-N'}} locator (sample={keys[:3]})",
            all(isinstance(k, str) and k.startswith(f"{project_key}-") for k in keys if k),
        )
        ...
        check(
            f"T2 status metadata attached to chunks ({len(statuses)} distinct values)",
            len(statuses) >= 1 and not all(s is None for s in statuses),
        )
        check(
            f"T2 priority metadata attached to chunks ({len(priorities)} distinct values)",
        ...
        for c in chunks:
            ...
            tokens += [
                w
                for w in _re.findall(r"[A-Za-z][A-Za-z0-9]{6,18}", text)
                if w.lower()
                not in {
                    "summary",
                    "description",
        ...
        if tokens:
            ...
            on_jira = [h for h in hits if (h.get("source") or "").endswith(issues_uri)]
            check(
                f"T3 search('{term}') surfaces hits in the project's issues "
        ...
        row_only = await eng.search(
            "issue", connector_uri=conn_uri, mode="hybrid", top_k=5, chunk_kinds=["row_text"]
        )
        check(
            f"T4 every filtered hit is chunk_kind='row_text' ({len(row_only)} hits)",
            len(row_only) == 0
            or all((h.get("metadata") or {}).get("chunk_kind") == "row_text" for h in row_only),
        )
        ...
        check(
            f"T5 scoped hits all live under /projects/{project_key}/ ({len(scoped)} hits)",
            len(scoped) == 0
            or all(f"/projects/{project_key}/" in (h.get("source") or "") for h in scoped),
        )
        ...
        if sample_key:
            cat_res = await eng.cat(conn_uri + issues_uri, locator={"key": sample_key})
            recd = _json.loads(cat_res["content"])
            check(
                f"T6 cat --locator {{'key': '{sample_key}'}} returns the issue",
                isinstance(recd, dict) and recd.get("key") == sample_key,
            )
        else:
        ...
        check(
            f"T7 second sync adds <= 2 new body tasks (real workspace; got {delta_tasks})",
            delta_tasks <= 2,
        )
        ...
        # repeat embeddings.
        check(
            f"T7 second sync: embedding API calls bounded by new content "
            f"(api delta={eng.embed.api_calls})",
            eng.embed.api_calls <= 5,
        )
        ...
        check(
            f"T8 ls / lists 'projects' dir + 'users.jsonl' (got {sorted(root_names)})",
            {"projects", "users.jsonl"} <= root_names,
        )
        ls_proj = await eng.ls(conn_uri + "/projects")
        proj_names = {e["name"] for e in ls_proj["entries"]}
        check(
            f"T8 ls /projects contains {project_key} (got {sorted(proj_names)})",
        ...
        sample_chunk = next((c for c in chunks if (c.get("locator") or {}).get("key")), None)
        if sample_chunk:
            ...
            check(
                f"T9 flatten_issue exposes the documented field set "
        ...
        pag_cid = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri='jira://t14-paginate'"
            )
        )["id"]
        ...
        check(
            f"T10 multi-page sync produced > 100 chunks (proves nextPageToken "
        ...
        cap_cid = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri='jira://t14-chunkcap'"
            )
        )["id"]
        ...
        check(
            f"T11 chunk_max=5 caps chunk_count (got {cap_ro['chunk_count'] if cap_ro else None})",
            cap_ro and cap_ro["chunk_count"] == 5,
        )
        check(
            f"T11 chunk_max truncation flips search_status='partial' "
            f"(got {cap_ro['search_status'] if cap_ro else None!r})",
            cap_ro and cap_ro["search_status"] == "partial",
        )
        cap_hits = await eng.search(
            "project", connector_uri="jira://t14-chunkcap", mode="hybrid", top_k=5
        )
        check(f"T11 partial slice still searchable ({len(cap_hits)} hits)", len(cap_hits) >= 1)
        ...
        check(
            f"T12 cat --range (0,3) returns 3 newline-joined records (got {len(decoded)})",
            len(decoded) == 3,
        )
        check(
            f"T12 each record carries the issue's 'key' field",
            all(
                isinstance(r.get("key"), str) and r["key"].startswith(f"{project_key}-")
                for r in decoded
            ),
        )
        ...
        if tokens:
            kw_hits = await eng.search(term, connector_uri=conn_uri, mode="keyword", top_k=5)
            sem_hits = await eng.search(term, connector_uri=conn_uri, mode="semantic", top_k=5)
            check(
                f"T13 keyword mode returns hit(s) containing the literal "
                f"term '{term}' ({len(kw_hits)} hits)",
                any(term.lower() in (h.get("content") or "").lower() for h in kw_hits),
            )
            check(f"T13 semantic mode also returns hits ({len(sem_hits)} hits)", len(sem_hits) >= 1)
        ...
        check(
            "T14 flatten_issue output has NO 'comment' or 'attachment' field "
            "(plugin doesn't request expand=comments; gap is intentional today)",
            sample_chunk is not None
            and "comment" not in rec
            and "attachment" not in rec
            and "comments" not in rec
            and "attachments" not in rec,
        )
