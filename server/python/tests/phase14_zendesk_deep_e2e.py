...
async def main():
    ...
    extra_comment_text = (
        "Followup: confirmed the rotation now uses ECDSA P-384 keys via vault.")

    async with httpx.AsyncClient(timeout=30) as c:
        # Seed 4 tickets
        for t in seeded:
            r = await c.post(f"{base_url}/api/v2/tickets.json", auth=auth,
                              json={"ticket": {
                                  "subject": t["subject"],
                                  "comment": {"body": t["body"]},
                                  "priority": t["priority"]}})
        ...
        r = await c.put(f"{base_url}/api/v2/tickets/{created_ids[0]}.json",
                         auth=auth, json={"ticket": {
                             "comment": {"body": extra_comment_text,
                                          "public": True}}})
        r.raise_for_status()

    base = f"/tmp/mfs_zd14_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        want = {"/tickets/records.jsonl", "/users/records.jsonl",
                 "/organizations/records.jsonl"}
        check(f"T1 tickets/users/organizations all surface "
        ...
        check(f"T1 tickets indexed (got {tk_chunks} chunks, expect >= 4 from seed)",
        ...
        check(f"T2 /tickets/comments.jsonl indexed "
              f"(chunks={cm_ro.get('chunk_count')})",
              (cm_ro.get("chunk_count") or 0) >= 1)
        # search for the unique comment string we added
        cm_hits = await eng.search("ECDSA P-384 keys via vault",
                                    connector_uri=conn_uri, mode="hybrid", top_k=5)
        check(f"T2 extra comment text surfaces in search "
              f"({len(cm_hits)} hits)",
        ...
        seeded_chunks = [c for c in ticket_chunks
                          if "T14 MFSTEST" in (c.get("content") or "")]
        prios = {(c.get("metadata") or {}).get("priority") for c in seeded_chunks}
        statuses = {(c.get("metadata") or {}).get("status") for c in seeded_chunks}
        check(f"T3 priority metadata covers the four seeded priorities "
              f"(got {prios})",
              prios >= {"high", "urgent", "normal", "low"})
        check(f"T3 status metadata attached to seeded tickets (got {statuses})",
              all(s in {"new", "open", "pending", "hold", "solved", "closed"}
                  for s in statuses if s))

        # ----- T4: search hits across tickets -----
        print("\n--- T4 · search across tickets ---")
        sso_hits = await eng.search("saml session token redirect persistence",
                                     connector_uri=conn_uri, mode="hybrid",
                                     top_k=5)
        check(f"T4 SSO ticket searchable ({len(sso_hits)} hits)",
              any("T14 MFSTEST sso" in (h.get("content") or "").lower()
                  or "saml" in (h.get("content") or "").lower()
                  for h in sso_hits))

        # ----- T5: cat --locator round-trips on a ticket id -----
        print("\n--- T5 · cat --locator round-trips ---")
        sample = next((c for c in seeded_chunks
                        if isinstance(c.get("locator"), dict)
                        and "id" in c["locator"]), None)
        if sample is None:
            sample = next((c for c in seeded_chunks
                            if isinstance(c.get("locator"), dict)), None)
        if sample:
            loc = sample["locator"]
            cat_res = await eng.cat(conn_uri + "/tickets/records.jsonl",
                                     locator=loc)
            recd = _json.loads(cat_res["content"])
            check(f"T5 cat --locator reopens the ticket ({loc})",
                  isinstance(recd, dict) and "subject" in recd
                  and "T14 MFSTEST" in (recd.get("subject") or ""))
        else:
            check("T5 no seeded chunk carried a locator — preset misconfigured?",
                  False)
        ...
        cap_cid = (await eng.meta.fetchone(
            "SELECT id FROM connectors WHERE root_uri='zendesk://t14-cap'"))["id"]
        cap_ro = await eng.meta.fetchone(
            ...
            (cap_cid,))
        check(f"T6 cap=2 flags search_status='partial' "
        ...
        cap_hits = await eng.search("MFSTEST",
                                     connector_uri="zendesk://t14-cap",
                                     mode="hybrid", top_k=5)
        check(f"T6 'partial' slice still searchable ({len(cap_hits)} hits)",
              len(cap_hits) >= 1)
        ...
        check(f"T6 finding: chunk_count > max_read_rows because cap is page-granular "
        ...
        check(f"T7 ls/ contains tickets,users,organizations (got {sorted(names)})",
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            for tid in created_ids:
                try: await c.delete(f"{base_url}/api/v2/tickets/{tid}.json",
                                     auth=auth)
                except Exception: pass
