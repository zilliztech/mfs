...
async def main():
    ...
    await asyncio.to_thread(
        cur.executemany, f"INSERT INTO {tbl} (ID, SUBJECT, BODY) VALUES (%s, %s, %s)", seed
    )

    # ---- Phase 2: drive MFS engine with snowflake connector ----
    base = f"/tmp/mfs_sfd_{os.getpid()}"
    ...
    try:
        ...
        check(
            f"rows.jsonl indexed with >=3 chunks (got {ro['chunk_count'] if ro else 'None'})",
        ...
        check(
            "NULL-column row still searchable (id=9003)",
        ...
        check(
            "modify reflected in search (id=9002 with new body)",
        ...
        check(
            "deleted row absent (id=9001 gone)",
