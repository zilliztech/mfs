...
async def main():
    ...
    base = f"/tmp/mfs_notion_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        page_rows = await eng.meta.fetchall(
            ...
            "WHERE connector_id=? AND object_uri LIKE '/pages/%' ORDER BY object_uri", (cid,))
        check(f"at least 1 page indexed (got {len(page_rows)} pages from the workspace)",
              len(page_rows) >= 1)
        ...
        check(f"at least 1 NON-EMPTY page has indexed chunks (got {len(non_empty)} non-empty / {len(page_rows)} total)",
              len(non_empty) >= 1)
        ...
        if non_empty:
            ...
            check(f"cat returns non-empty markdown for {sample['object_uri']} ({len(content) if isinstance(content, str) else '?'} chars)",
            ...
            check("head -n 5 returns text for the same page",
            ...
            if words:
                ...
                check(f"search for content-derived term {q!r} finds at least 1 hit on this connector",
