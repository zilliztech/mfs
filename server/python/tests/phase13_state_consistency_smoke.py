...
async def main():
    ...
    base = f"/tmp/mfs_state_{os.getpid()}"
    ...
    try:
        ...
        await eng.add(root)
        cid = (
            await eng.meta.fetchone("SELECT id, status FROM connectors WHERE root_uri=?", (cu,))
        )["id"]
        ...
        check(
            "objects: one row per file (4)",
            ouris == {"/notes.md", "/app.py", "/pic.png", "/data.bin"},
        )
        ...
        for o in objs:
            ...
            if mc != o["chunk_count"]:
                consistent = False
                print(f"      mismatch {o['object_uri']}: objects={o['chunk_count']} milvus={mc}")
        ...
        check(
            "Milvus total == file chunks + dir summaries",
            total == sum(o["chunk_count"] for o in objs) + ndir,
        )

        # tasks: all terminal-succeeded, none stuck
        tk = await eng.meta.fetchall(
            "SELECT status, count(*) AS n FROM object_tasks WHERE connector_id=? GROUP BY status",
            (cid,),
        )
        tmap = {r["status"]: r["n"] for r in tk}
        check(
            "object_tasks: all succeeded, none pending/failed/cancelled",
            tmap.get("succeeded", 0) > 0
            and not (
                tmap.get("pending")
                or tmap.get("running")
                or tmap.get("failed")
                or tmap.get("cancelled")
            ),
        )
        ...
        job = await eng.meta.fetchone(
            ...
            "WHERE connector_id=? ORDER BY started_at DESC LIMIT 1",
            (cid,),
        )
        check(
            "job: succeeded, total==succeeded, 0 failed",
            job["status"] == "succeeded"
            and job["total_objects"] == job["succeeded_objects"]
            and job["failed_objects"] == 0,
        )
        ...
        check(
            "modify: app.py objects.chunk_count == Milvus count",
            mc_app == await _obj_count(eng, cid, "/app.py"),
        )
        res = await eng.search("gateway refund invoice", connector_uri=cu, mode="hybrid", top_k=5)
        check(
            "modify: search reflects new content", any("app.py" in (e["source"] or "") for e in res)
        )
        ...
        check(
            "delete: Milvus chunks for pic.png purged",
            await _mcount(eng, f'object_uri == "{_lit(full("/pic.png"))}"') == 0,
        )
        art2 = await eng.meta.fetchone(
            "SELECT count(*) AS n FROM artifact_cache WHERE object_uri=?", (full("/pic.png"),)
        )
        ...
        for t in ("objects", "object_tasks", "connector_jobs", "connector_state", "file_state"):
            r = await eng.meta.fetchone(
                f"SELECT count(*) AS n FROM {t} WHERE connector_id=?", (cid,)
            )
            if r["n"]:
                leftovers[t] = r["n"]
        ...
        if r["n"]:
            leftovers["connectors"] = r["n"]
        # artifact_cache keyed by object_uri -> purge by connector_uri prefix
        r = await eng.meta.fetchone(
            "SELECT count(*) AS n FROM artifact_cache WHERE object_uri LIKE ?", (cu + "%",)
        )
        if r["n"]:
            leftovers["artifact_cache"] = r["n"]
