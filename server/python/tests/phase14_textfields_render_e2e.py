...
async def main():
    ...
    check(f"A.1 single column renders as 'field: value' ({got!r})",
    ...
    check(f"A.4 missing field skipped silently ({got!r})",
          got == "subject: only this")
    ...
    nested = {"payload": {"title": "alpha",
                           "body": "cytochrome oxidase activity"}}
    got = _render_record(nested, ["payload.body"])
    check(f"A.6 nested 'payload.body' renders the inner string ({got!r})",
          got == "payload.body: cytochrome oxidase activity")

    # A.7: multi-valued array path -> bulleted list under the field header
    arr = {"comments": [{"body": "first reply"},
                         {"body": "second reply"},
                         {"body": "third reply"}]}
    got = _render_record(arr, ["comments[].body"])
    check(f"A.7 array path 'comments[].body' flattens to bulleted list "
          f"({got!r})",
          got == "comments[].body:\n- first reply\n- second reply\n- third reply")

    # A.8: mix scalar + array
    mixed = {"subject": "deploy rollback",
             "comments": [{"body": "started rollback"},
                           {"body": "rollback complete"}]}
    got = _render_record(mixed, ["subject", "comments[].body"])
    check(f"A.8 mixed scalar + array preserves order + bullets the array "
          f"({got!r})",
          got == "subject: deploy rollback\n\ncomments[].body:\n- started rollback\n- rollback complete")

    # =====================================================
    # B · _resolve_path edges
    # =====================================================
    print("\n--- B · _resolve_path edges ---")
    check("B.1 missing key returns None",
          _resolve_path({"a": 1}, "missing") is None)
    check("B.2 nested missing returns None",
          _resolve_path({"a": {"b": 1}}, "a.c") is None)
    check("B.3 scalar nested path",
          _resolve_path({"a": {"b": "x"}}, "a.b") == "x")
    check("B.4 array index path",
          _resolve_path({"a": [{"b": 1}, {"b": 2}]}, "a[0].b") == 1)
    check("B.5 array wildcard returns list",
          _resolve_path({"a": [{"b": 1}, {"b": 2}]}, "a[*].b") == [1, 2])
    check("B.6 None vs empty-string distinguishable (None -> None)",
          _resolve_path({}, "a") is None)
    ...
    import aiomysql
    try:
        conn = await aiomysql.connect(autocommit=True, host="127.0.0.1",
                                       port=3306, user="mfs", password="mfs",
                                       db="mfstest")
    ...
    await cur.execute(
        f"CREATE TABLE `{tbl}` (id INT PRIMARY KEY, subject TEXT, body TEXT, "
        "priority VARCHAR(20))")
    src_row = (1, "T14 multi-column rendering",
               "We pin the contract that text_fields joins into the single "
               "Milvus content field with a blank-line separator.",
               "high")
    await cur.execute(
        f"INSERT INTO `{tbl}` (id, subject, body, priority) "
        f"VALUES (%s, %s, %s, %s)", src_row)

    base = f"/tmp/mfs_tfrender_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    cfg = load_server_config(apply_env=False)
    cfg.metadata.path = base + "_m.db"
    cfg.milvus.uri = base + "_v.db"; cfg.milvus.token = ""
    ...
    try:
        ...
        check(f"C.1 exactly one Milvus chunk for the row ({len(chunks)})",
              len(chunks) == 1)
        chunk = chunks[0]

        # Reconstruct the dict the framework saw at sync time
        src_dict = {"id": 1, "subject": src_row[1],
                     "body": src_row[2], "priority": src_row[3]}
        expected_content = _render_record(src_dict, ["subject", "body"])
        check(f"C.2 Milvus.content is the _render_record output "
              f"(rendered={expected_content!r})",
        ...
        check("C.3 rendered content carries both 'subject:' and 'body:' headers",
              "subject:" in chunk["content"]
              and "body:" in chunk["content"]
              and "\n\n" in chunk["content"])
        check("C.4 priority did NOT bleed into content (it's metadata, not text)",
        ...
        check(f"C.5 locator JSON column = {{'id': 1}} (got {loc})",
              loc == {"id": 1})
        check(f"C.6 metadata JSON column = {{'priority': 'high'}} (got {meta})",
              meta == {"priority": "high"})
        ...
        rendered_key = cache_key(
            ...
            "embedding", eng.embed.provider, eng.embed.model, eng.embed.version)
        cached = await eng.tx_cache.batch_get([rendered_key])
        check(f"D.1 transformation_cache has an entry under "
        ...
        check(f"D.2 re-embedding the rendered string is a cache hit "
        ...
        check(f"D.3 embedding just one column is a fresh miss "
              f"(api delta={eng.embed.api_calls})",
