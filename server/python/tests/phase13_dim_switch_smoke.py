...
async def main():
    ...
    try:
        ...
        check(
            "dim switch -> different collection name",
            coll_512 != coll_1536 and coll_512.endswith("_d512"),
        )
        check("old collection still exists", eng.milvus.client.has_collection(coll_1536))
        check(
            "new-dim collection does not exist yet", not eng.milvus.client.has_collection(coll_512)
        )
        # old collection untouched (still has the original vectors)
        old_cnt = eng.milvus.client.query(
            collection_name=coll_1536,
            filter="chunk_id != ''",
            output_fields=["count(*)"],
