...
def main():
    ...
    check("walk_tree: ignored globs/dirs excluded (b.log, node_modules, build, .git, secret_, .tmp)",
          not any(x in native for x in ("/b.log", "/node_modules/pkg/index.js", "/build/out.o",
                                        "/.git/config", "/secret_key.txt", "/src/scratch.tmp")))
    check("walk_tree: negated keep.log included", "/keep.log" in native)
    check("walk_tree: normal files included", {"/a.py", "/src/app.py", "/src/sub/util.py", "/readme.md"} <= set(native))
    check("walk_tree: size/mtime_ns/inode match os.stat for every file",
          all(native[k] == ref[k] for k in ref))

    # sha1_files parity
    sf_paths = [f"{root}/{r}" for r in ("a.py", "src/app.py", "readme.md")]
    open(f"{root}/empty.txt", "w").close(); sf_paths.append(f"{root}/empty.txt")
