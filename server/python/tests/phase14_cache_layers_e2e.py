...
async def _web_handler(request: web.Request) -> web.Response:
    WEB_HITS["count"] += 1
    if request.path == "/":
        return web.Response(text=HTML_A, content_type="text/html",
                            headers={"ETag": '"v1"'})
...
async def main():
    ...
    base = f"/tmp/mfs_cache_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        repo_a = tmp / "repoA"; repo_a.mkdir()
        (repo_a / "alpha.html").write_text(HTML_A)
        eng.converter.api_calls = 0; eng.converter.cache_hits = 0
        await eng.add(str(repo_a))
        cidA = (await eng.meta.fetchone(
            "SELECT id, root_uri FROM connectors WHERE root_uri=?",
            (f"file://local{repo_a}",)))["id"]
        ...
        check("A.1 artifact_cache row exists for converted_md",
              row is not None and row["size_bytes"] > 0)
        check("A.1 storage_path resolves to real bytes on disk",
              row and os.path.exists(row["storage_path"]))
        check(f"A.1 first sync ran the converter exactly once "
              f"(api_calls={eng.converter.api_calls})",
        ...
        row2 = await eng.meta.fetchone(
            ...
            (ns, full_a, "converted_md"))
        check("A.2 cat returns the cached markdown",
              isinstance(md1, str) and "Photosynthesis" in md1 and md1 == md2)
        check(f"A.2 cat path did NOT re-run markitdown "
              f"(api_calls={eng.converter.api_calls})", eng.converter.api_calls == 0)
        check(f"A.2 last_accessed advanced on cat ({first_accessed!r} -> {row2['last_accessed']!r})",
              row2["last_accessed"] >= first_accessed)
        check("A.2 artifact size unchanged after re-reads",
              row2["size_bytes"] == row["size_bytes"])

        # A.3 — head() on a structured JSONL writes head_cache
        repo_jsonl = tmp / "repoJ"; repo_jsonl.mkdir()
        ...
        head_row = await eng.meta.fetchone(
            ...
            "WHERE namespace_id=? AND artifact_kind='head_cache'", (ns,))
        check("A.3 head_cache artifact_kind reachable in schema (smoke)",
              True)  # The structured path needs a DB connector — A.3 stays a smoke
        del head_row  # noqa: F841

        # A.4 — web add: converted_md persisted; remove drops it
        WEB_HITS["count"] = 0; WEB_MD_CALLS["count"] = 0
        await eng.add("web://cache-a4", config={"start_urls": [f"{web_origin}/"],
                                                 "allowed_domains": [f"127.0.0.1:{web_port}"],
                                                 "max_pages": 1})
        check(f"A.4 web fetch ran once on add (hits={WEB_HITS['count']})",
              WEB_HITS["count"] == 1)
        check(f"A.4 web markitdown ran once on add (calls={WEB_MD_CALLS['count']})",
              WEB_MD_CALLS["count"] == 1)
        web_rows = await eng.meta.fetchall(
            ...
            "AND object_uri LIKE 'web://cache-a4%'", (ns,))
        check(f"A.4 web converted_md artifact persisted ({len(web_rows)} row)",
        ...
        check(f"A.4 cat on web page returns cached markdown",
              isinstance(cat_web, str) and "Photosynthesis" in cat_web)
        check(f"A.4 cat did NOT re-issue an HTTP fetch "
        ...
        after_remove = await eng.meta.fetchall(
            ...
            (ns,))
        check(f"A.4 remove drops artifact_cache rows ({len(after_remove)} left)",
              len(after_remove) == 0)
        check("A.4 remove drops object_store bytes",
              not os.path.exists(artifact_path_to_check))
        ...
        repo_lru = tmp / "repoLRU"; repo_lru.mkdir()
        ...
        before = await eng.meta.fetchall(
            ...
            "AND artifact_kind='converted_md' ORDER BY last_accessed ASC", (ns, uri_lru + "%"))
        total_bytes = sum(r["size_bytes"] for r in before)
        check(f"A.5 LRU pre: {len(before)} artifacts totalling {total_bytes} bytes",
        ...
        check(f"A.5 eviction ran (evicted={evicted}, rows {len(before)} -> {len(after_rows)})",
              evicted >= 1 and len(after_rows) < len(before))
        check(f"A.5 total bytes after eviction <= cap ({after_bytes} <= {cap_bytes})",
              after_bytes <= cap_bytes)
        check("A.5 the 'recent' row survived (LRU is by last_accessed ASC)",
              any(r["object_uri"] == recent["object_uri"] for r in after_rows))
        survivor_paths = {r["storage_path"] for r in after_rows}
        evicted_paths = [r["storage_path"] for r in before
                         if r["storage_path"] not in survivor_paths]
        check(f"A.5 evicted artifacts' bytes deleted from object_store ("
        ...
        repo_b1a = tmp / "repoB1A"; repo_b1a.mkdir()
        repo_b1b = tmp / "repoB1B"; repo_b1b.mkdir()
        ...
        eng.embed.api_calls = 0; eng.embed.cache_hits = 0
        ...
        check(f"B.1 first repo embeds via API ({b1a_api} calls, {b1a_hit} hits)",
              b1a_api >= 1)
        check(f"B.1 second repo (identical .md) hits cache, no API "
        ...
        k1 = cache_key(sha1_hex(shared_text.encode()), "embedding",
                       eng.embed.provider, eng.embed.model, eng.embed.version)
        row_for_k1 = await _tx_row_for(k1)
        check("B.2 the shared embedding key lives in tx_cache",
              row_for_k1 is not None)
        ...
        k_alt = cache_key(sha1_hex(shared_text.encode()), "embedding",
                          eng.embed.provider, "text-embedding-3-large",
                          eng.embed.version)
        check(f"B.3 changing model name yields a different cache_key "
              f"({eng.embed.model} vs text-embedding-3-large)", k1 != k_alt)
        miss = await _tx_row_for(k_alt)
        check("B.3 the alternate-model key is NOT in tx_cache (cold miss)",
              miss is None)

        # B.4 — convert (markitdown via eng.converter) cache hit on identical bytes
        repo_b4a = tmp / "repoB4A"; repo_b4a.mkdir()
        repo_b4b = tmp / "repoB4B"; repo_b4b.mkdir()
        ...
        check(f"B.4 first convert calls markitdown ({b4a_api} calls)",
              b4a_api == 1)
        check(f"B.4 second connector (same bytes) hits convert cache "
        ...
        repo_b5a = tmp / "repoB5A"; repo_b5a.mkdir()
        repo_b5b = tmp / "repoB5B"; repo_b5b.mkdir()
        summary_body = ("# Module README\n\nThis module wraps the auth pipeline:\n"
                        "- token verification\n- session minting\n- audit log emit\n")
        ...
        eng.summary.api_calls = 0; eng.summary.cache_hits = 0
        ...
        check(f"B.5 first directory_summary calls LLM ({b5a_api} calls, "
              f"{b5a_hit} hits)", b5a_api >= 1)
        check(f"B.5 second connector (same dir body) hits summary cache "
        ...
        check(f"C.0 first sync hashes the new file (sha1 batch totals={SHA1_CALLS})",
              first_sha1_total >= 1)
        ...
        check(f"C.1 unchanged file: re-sync's sha1 batches are empty "
        ...
        check(f"C.2 mtime-only touch: sha1 IS recomputed (paths hashed={c2_total})",
              c2_total >= 1)
        check("C.2 mtime-only touch: object_uri set is unchanged",
              set(before_by_uri) == set(after_by_uri))
        check("C.2 mtime-only touch: fingerprint (sha1) unchanged "
              "-> engine treats as same content",
              all(after_by_uri[k]["fingerprint"] == v["fingerprint"]
        ...
        check("C.3 content change: fingerprint advanced (sha1 differs)",
              any(post_by_uri[k]["fingerprint"] != v["fingerprint"]
                  for k, v in before_by_uri.items() if k in post_by_uri))
