...
def main():
    ...
    cfg.milvus.token = ""
    cfg.object_store.root = base + "_store"
    ...
    with TestClient(app) as client:
        ...
        r = client.post(
            "/v1/upload?name=acme-bundle", content=tar, headers={"content-type": "application/gzip"}
        )
        ...
        check(
            "search finds the uploaded auth.md",
        ...
        if top:
            ...
            check(
                "upload identity is file://acme-bundle",
                top["source"].startswith("file://acme-bundle/"),
            )
            cat = client.get("/v1/cat", params={"path": top["source"]})
            check(
                "cat reads staged file content", "Single sign-on" in cat.json().get("content", "")
            )
        else:
        ...
        er = client.post(
            "/v1/upload?name=evil", content=evil, headers={"content-type": "application/gzip"}
        )
