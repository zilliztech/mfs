...
async def _drive(eng, conn_uri, cfg_obj, *, label, expect_chunks):
    ...
    if expect_chunks:
        ...
        check(f"[{label}] at least one channel has >=1 thread_aggregate chunk "
        ...
        res = await eng.search("project meeting", connector_uri=conn_uri,
                               mode="hybrid", top_k=3)
        on_us = [r for r in res if (r.get("source") or "").startswith(conn_uri)]
        check(f"[{label}] search returns >=1 hit (got {len(on_us)})", len(on_us) >= 1)
    else:
        check(f"[{label}] bot in 0 channels (expected — bot not invited anywhere)",
              len(chan_objs) == 0)
...
async def main():
    ...
    base = f"/tmp/mfs_slack_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
