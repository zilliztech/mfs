...
async def main():
    ...
    cases = [
        ...
        # docs (-190)
        ("/docs/intro.md", -190),
        ("/guides/quickstart.md", -190),
        # tests (+80)
        ("/tests/test_auth.py", 80),
        ("/test/foo.py", 80),
        ("/__tests__/bar.js", 80),
        ("/fixtures/data.json", 80),
        # generated (+260)
        ("/dist/bundle.js", 260),
        ("/build/out.o", 260),
        ("/vendor/lib.so", 260),
        ("/node_modules/pkg/index.js", 260),
        ("/target/release/main", 260),
        ("/out/foo.js", 260),
        # default (0)
        ("/notes.md", 0),
        ("/random.py", 0),
    ...
    base = f"/tmp/mfs_prog_{os.getpid()}"
    ...
    # Tests (priority +80)
    (repo / "tests" / "test_auth.py").write_text("def test_verify_saml():\n    assert True\n")
    ...
    try:
        ...
        for path, want in expected_buckets.items():
            ...
            check(f"object_tasks: priority({path}) = {want:+d} (got {got})", got == want)
        ...
        check("ORDER BY: pyproject < src/auth.py", _p("/pyproject.toml") < _p("/src/auth.py"))
        check("ORDER BY: src/auth.py < docs/intro.md", _p("/src/auth.py") < _p("/docs/intro.md"))
        check(
            "ORDER BY: docs/intro.md < data/metrics.json (generic 0)",
            _p("/docs/intro.md") < _p("/data/metrics.json"),
        )
        check(
            "ORDER BY: data/metrics.json (0) < tests/test_auth.py",
        ...
        check(
            f"dir_summary tasks present and use priority=-depth (got {sorted(set(dir_prios))})",
        ...
        db_cid = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri='mysql://progressive'"
            )
        )["id"]
        ...
        check(
            f"chunk_max=5 caps DB chunk_count (got {db_obj['chunk_count']})",
            db_obj["chunk_count"] == 5,
        )
        check(
            f"chunk_max truncation flags search_status='partial' (got {db_obj['search_status']!r})",
            db_obj["search_status"] == "partial",
        )
        # search returns hits from the partial slice — i.e. "partial" doesn't mean unavailable
        db_res = await eng.search(
            "topic-0", connector_uri="mysql://progressive", mode="hybrid", top_k=5
        )
        check(
            f"search hits from the 'partial' indexed slice ({len(db_res)} hits)", len(db_res) >= 1
        )
        ...
        check(
            f"file connector status aggregation: indexed={agg_d.get('indexed', 0)}, "
