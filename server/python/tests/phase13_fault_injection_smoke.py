...
def _mkcfg(base):
    ...
    cfg.worker.backoff_initial_ms = 5
    cfg.worker.backoff_max_ms = 10  # keep retries fast
    return cfg


async def _job_tasks(eng, job):
    rows = await eng.meta.fetchall(
        "SELECT status, last_error FROM object_tasks WHERE connector_job_id=?", (job,)
    )
    return rows


async def case_transient():
    base = f"/tmp/mfs_fj_t_{os.getpid()}"
    ...
    try:
        ...
        check(
            "J1 transient: task eventually succeeded",
            all(t["status"] == "succeeded" for t in tk) and len(tk) == 1,
        )
...
async def case_fatal():
    base = f"/tmp/mfs_fj_f_{os.getpid()}"
    os.system(f"rm -rf '{base}'*")
...
async def case_partial():
    base = f"/tmp/mfs_fj_p_{os.getpid()}"
    os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        row = await eng.meta.fetchone(
            "SELECT succeeded_objects, failed_objects, total_objects FROM connector_jobs WHERE id=?",
            (job,),
        )
        check("J8 partial: 1 succeeded", row["succeeded_objects"] == 1)
        check("J8 partial: 1 failed", row["failed_objects"] == 1)
        res = await eng.search(
            "this one indexes fine", connector_uri=f"file://local{root}", mode="hybrid", top_k=5
        )
        check(
            "J8 partial: good object still retrievable",
            any("good.md" in (e["source"] or "") for e in res),
        )
    finally:
        eng.embed.batch_embed = orig
        try:
...
async def case_breaker():
    base = f"/tmp/mfs_fj_b_{os.getpid()}"
    os.system(f"rm -rf '{base}'*")
    root = tempfile.mkdtemp(prefix="mfs_fj_b_")
    for i in range(8):
        open(f"{root}/f{i}.md", "w").write(f"# F{i}\n\ncontent {i}\n")
    cfg = _mkcfg(base)
    cfg.worker.consecutive_fatal_threshold = 3
    eng = Engine(cfg)
    await eng.startup()
