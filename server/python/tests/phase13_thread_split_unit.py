...
def main():
    ...
    check(
        "short thread sub-chunk contains every message body",
    ...
    check(
        "every sub-chunk <= max_chars (allowing 1 msg overflow)",
        all(len(t) <= 1500 + 410 for _, _, t in sub),
    )  # 1 msg can push slightly past cap
    check(
        "sub-chunks cover the full thread (start at 0, end at 19)",
    ...
    for i in range(len(sub) - 1):
        ...
        check(
            f"sub-chunk {i + 1} starts {_THREAD_OVERLAP_MESSAGES} msgs into the previous one "
