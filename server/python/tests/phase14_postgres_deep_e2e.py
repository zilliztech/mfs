...
def _dt(s: str) -> datetime:
    ...
    return datetime.strptime(s.replace("+00", "+0000"),
                              "%Y-%m-%d %H:%M:%S%z").astimezone(timezone.utc)
...
async def _seed(conn: asyncpg.Connection):
    ...
    """)
    await conn.executemany(
        "INSERT INTO public.t14_jsonb (id, payload, updated_at) "
        "VALUES ($1, $2::jsonb, $3)",
        [(1, _json.dumps({"title": "infra-week", "body":
            "mitochondrial cristae morphology shifts under hypoxia"}),
          _dt("2026-05-01 10:00:00+00")),
         (2, _json.dumps({"title": "ops-followup", "body":
            "carbon-13 NMR identifies the diastereomer ratio at 84/16"}),
          _dt("2026-05-01 10:00:00+00"))])
    ...
    await conn.executemany(
        ...
        [(1, "eddy current loss dominates at this switching frequency",
          _dt("2026-05-01 10:00:00+00")),
         (2, "thermal runaway mitigated by current foldback",
          _dt("2026-05-01 10:00:00+00"))])
...
async def main():
    ...
    try:
        ...
        print(f"postgres not reachable at {DSN}: {e}"); raise SystemExit(2)

    await _drop_all(conn)
    await _seed(conn)

    base = f"/tmp/mfs_pgdeep_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg_obj = {
        ...
        "objects": [
            ...
            {"match": "/t14ops/notes/rows.jsonl",
             "text_fields": ["note"], "locator_fields": ["id"]},
        ],
    }
    ...
    try:
        ...
        expected_objects = {
            "/public/t14_tickets/rows.jsonl", "/public/t14_tickets/schema.json",
            "/public/t14_big/rows.jsonl",     "/public/t14_big/schema.json",
            "/public/t14_compkey/rows.jsonl", "/public/t14_compkey/schema.json",
            "/public/t14_jsonb/rows.jsonl",   "/public/t14_jsonb/schema.json",
            "/t14ops/notes/rows.jsonl",       "/t14ops/notes/schema.json",
        }
        check(f"T1 all 10 expected objects landed "
              f"(missing={sorted(expected_objects - set(uris))})",
              expected_objects <= set(uris))
        check("T1 t14ops schema's table indexed (not only public)",
        ...
        check(f"T2 composite locator carries BOTH PK columns (got {loc})",
              isinstance(loc, dict) and set(loc.keys()) >= {"region", "user_id"}
              and loc["region"] == "us-east-1" and loc["user_id"] == 200)
        ...
        check("T2 cat --locator composite key returns exact record",
              rec.get("region") == "eu-west-1" and rec.get("user_id") == 100
        ...
        check(f"T3 raw jsonb column lands in row_text ({len(jb_chunks)} chunks)",
              len(jb_chunks) == 2
              and "mitochondrial cristae" in joined
              and "carbon-13 NMR" in joined)
        res_jb = await eng.search("mitochondrial cristae hypoxia",
                                   connector_uri=conn_uri, mode="hybrid", top_k=5)
        check(f"T3 search hit on the jsonb-derived text ({len(res_jb)} hits)",
        ...
        check(f"T3 finding: jsonb dotted-path unsupported when payload is str "
              f"(_resolve_path returned {deep!r})", deep is None)
        ...
        check(f"T4 priority metadata present on every chunk "
              f"(got {priorities})", priorities == {"high", "medium"})
        check(f"T4 assignee metadata present on every chunk "
              f"(got {assignees})", assignees == {"alice", "bob"})
        ...
        check(f"T5 grep pushdown returns row(s) matching 'webhook' "
              f"({len(grep_hits)} hits)",
              len(grep_hits) >= 1
              and any("Stripe webhook" in c for c in contents))
        # 'noisy neighbor' is in t14_compkey
        ck_grep = await eng.grep("noisy neighbor", ck_uri, top_k=20)
        check(f"T5 grep on a different table works too ({len(ck_grep)} hits)",
              len(ck_grep) >= 1)
        ...
        check(f"T6 new row id=4 indexed on re-sync (locators={sorted(ids)})",
              4 in ids)
        res6 = await eng.search("tls handshake ssl_error_handshake",
                                connector_uri=conn_uri, mode="hybrid", top_k=5)
        check(f"T6 new row searchable post-incremental ({len(res6)} hits)",
        ...
        check(f"T7 chunk_max=10 caps chunk_count "
              f"(got {big_ro['chunk_count']})",
              big_ro and big_ro["chunk_count"] == 10)
        check(f"T7 search_status='partial' (got {big_ro['search_status']!r})",
              big_ro and big_ro["search_status"] == "partial")
        big_uri = conn_uri + "/public/t14_big/rows.jsonl"
        partial_hits = await eng.search("topic-0", connector_uri=conn_uri,
                                         object_prefix=big_uri, mode="hybrid",
                                         top_k=5)
        check(f"T7 search hits within the 'partial' slice "
              f"({len(partial_hits)} hits)", len(partial_hits) >= 1)
        ...
        check(f"T8 schema.json produces schema_summary chunk(s) "
              f"(kinds={kinds}, n={len(sc_chunks)})",
              "schema_summary" in kinds)
        # The summary text mentions the column names
        joined_summary = " ".join((c.get("content") or "") for c in sc_chunks
                                   if c.get("chunk_kind") == "schema_summary")
        check(f"T8 schema_summary mentions the column names",
