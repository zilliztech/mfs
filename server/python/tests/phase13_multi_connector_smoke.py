...
async def main():
    ...
    da = tempfile.mkdtemp(prefix="mfs_mc_a_"); db = tempfile.mkdtemp(prefix="mfs_mc_b_")
    ...
    base = f"/tmp/mfs_mc_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        await eng.add(da); await eng.add(db)
        ...
        cid = (await eng.meta.fetchone("SELECT id FROM connectors WHERE root_uri=?", (f"file://local{da}",)))["id"]
        ...
        ra = await eng.search("raft leader election", connector_uri=f"file://local{da}", mode="hybrid", top_k=5)
        check("G2: removed connector returns nothing", ra == [] or not any("a.md" in (e["source"] or "") for e in ra))
        rb = await eng.search("columnar zonemap pruning", connector_uri=f"file://local{db}", mode="hybrid", top_k=5)
        check("G2: surviving connector still searchable", any("b.md" in (e["source"] or "") for e in rb))
        left = await eng.meta.fetchall("SELECT root_uri FROM connectors")
        check("G2: only the surviving connector remains", len(left) == 1 and db in left[0]["root_uri"])
    finally:
