...
from mfs_server.connectors.base import (
    Capabilities, ConnectorPlugin, Entry, GrepMatch, GrepOptions, ObjectChange,
    ObjectKind, PathStat, Range, SyncOptions,
)
...
class MemGrepPlugin(ConnectorPlugin):
    NAME = "memgrep"; URI_SCHEME = "memgrep"; DISPLAY_NAME = "grep (test)"; PROMPT = "t"
        ...
        async def gen():
            for r in _ROWS:
                if any(pattern.lower() in str(r.get(f, "")).lower() for f in options.text_fields):
                    yield GrepMatch(path=path, locator={"id": r["id"]},
                                    content=str(r.get("subject", "")))
        return gen()


async def main():
    registry.register(MemGrepPlugin)
    base = f"/tmp/mfs_gp_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    eng._resolve_target = lambda t: ("memgrep", t, "memgrep", {}) if t.startswith("memgrep://") else orig(t)
    await eng.startup()
    try:
        ...
        check("grep used connector pushdown (via=pushdown)", hits and all(h["via"] == "pushdown" for h in hits))
