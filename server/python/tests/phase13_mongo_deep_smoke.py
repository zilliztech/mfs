...
async def main():
        ...
        [
            {
                ...
                "body": "Users bounced to the identity provider",
                "updatedAt": now,
            },
            {
                "ticket_id": 2,
                "title": "Dark mode toggle",
                "body": "Add a dark theme switch",
                "updatedAt": now,
            },
    ...
    conn_uri = "mongo://deep"
    src = conn_uri + "/tickets/documents.jsonl"
    try:
        ...
        sc = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + "/tickets/schema.json"
        )
        check(
            "mongo schema_summary produced",
            any(c.get("chunk_kind") == "schema_summary" for c in sc),
        )
        res = await eng.search(
            "single sign-on identity provider", connector_uri=conn_uri, mode="hybrid", top_k=5
        )
        ...
        check(
            "mongo cat --locator exact record",
            recd.get("ticket_id") == 1 and "identity provider" in (recd.get("body") or ""),
        )
        try:
        ...
        check(
            "mongo NULL-field doc searchable",
            any((e.get("locator") or {}).get("ticket_id") == 3 for e in rn),
        )
        ...
        coll.update_one(
            ...
            {
                "$set": {
                    "body": "Now supports a high-contrast accessibility palette",
        ...
        check(
            "mongo modify reflected",
            any((e.get("locator") or {}).get("ticket_id") == 2 for e in rm),
        )
        coll.delete_one({"ticket_id": 1})
        await eng.add("mongo://deep", config=cfg_obj, full=False)
        rd = await eng.search(
            "single sign-on identity provider", connector_uri=conn_uri, mode="hybrid", top_k=10
        )
        check(
            "mongo deleted doc gone",
            not any((e.get("locator") or {}).get("ticket_id") == 1 for e in rd),
        )
    finally:
        try:
            eng.milvus.drop_collection("default")
