...
async def _tasks_for(eng, job_id):
    r = await eng.meta.fetchall("SELECT change_kind FROM object_tasks WHERE connector_job_id=?", (job_id,))
...
async def main():
    ...
    base = f"/tmp/mfs_life_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        check("D4 modify: search reflects new content", any("b.md" in (e["source"] or "") for e in res))
        ...
        gone = await eng.meta.fetchone(
            ...
            "WHERE c.root_uri=? AND o.object_uri='/c.md'", (conn_uri,))
        check("D7 delete: object row gone", gone is None)
        res = await eng.search("gamma module export", connector_uri=conn_uri, mode="hybrid", top_k=5)
        ...
        check("D12 force-index: rebuilt, 0 new embeddings (cache hit)", eng.embed.api_calls == calls2)
    finally:
