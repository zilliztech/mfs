...
async def main():
    ...
    base = f"/tmp/mfs_disc_inc_{os.getpid()}"
    ...
    try:
        ...
        cid = (await eng.meta.fetchone("SELECT id FROM connectors WHERE type='discord'"))["id"]
        chan_path = next(
            (
                r["object_uri"]
                for r in await eng.meta.fetchall(
                    "SELECT object_uri FROM objects WHERE connector_id=? AND object_uri LIKE ?",
                    (cid, f"%__{channel_id}/messages.jsonl"),
                )
            ),
            None,
        )
        ...
        check(
            f"tag {tag!r} not yet present in any chunk content "
        ...
        check(
            f"re-sync: chunk_count grew or stayed (baseline {baseline_chunks} -> {post_chunks})",
            post_chunks >= baseline_chunks,
        )
        check(
            f"re-sync: search_status remains 'indexed' (got {post_ro['search_status']!r})",
        ...
        check(
            f"after re-sync: chunk containing tag {tag!r} surfaces in search "
            f"({len(with_tag_1)} matching hits / {len(res1)} top-k)",
            len(with_tag_1) >= 1,
        )
        # also try a semantic query — should hit too
        res2 = await eng.search(
            "post-quantum lattice key encapsulation", connector_uri=conn_uri, mode="hybrid", top_k=5
        )
        on_us = [r for r in res2 if (r.get("source") or "").startswith(conn_uri)]
        check(
            f"after re-sync: semantic search finds the new content ({len(on_us)} hits)",
            len(on_us) >= 1,
        )
    finally:
        ...
        if posted_msg_id:
            try:
                await _delete_message(channel_id, posted_msg_id)
            except Exception:
                pass
        try:
            eng.milvus.drop_collection("default")
