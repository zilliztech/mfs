...
async def main():
    ...
    rows_uri = f"/{tbl}/rows.jsonl"
    src = "mysql://deep" + rows_uri
    conn_uri = "mysql://deep"
    try:
        ...
        check(
            "mysql schema_summary produced",
            any(c.get("chunk_kind") == "schema_summary" for c in sc),
        )

        res = await eng.search(
            "single sign-on bounced identity provider",
        ...
        check(
            "mysql cat --locator exact record",
        ...
        check(
            "mysql NULL-column row searchable",
            any((e.get("locator") or {}).get("id") == 9003 for e in rn),
        )

        # incremental modify + delete
        await cur.execute(
            f"UPDATE `{tbl}` SET body=%s WHERE id=9002",
