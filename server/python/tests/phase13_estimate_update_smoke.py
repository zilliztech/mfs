...
async def main():
    ...
    base = f"/tmp/mfs_eu_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        check("estimate returns physical quantities", est.get("est_chunks") is not None or est.get("objects") is not None)
        check("L3 estimate: ZERO embedding calls", eng.embed.api_calls == calls0)
        check("L3 estimate: ZERO Milvus chunks", await asyncio.to_thread(eng.milvus.count, "default") == 0)
        ...
        check("L4 estimate: no leaked connector/objects/file_state rows",
        ...
        cid = (await eng.meta.fetchone("SELECT id FROM connectors WHERE root_uri=?", (conn_uri,)))["id"]
        before = json.loads((await eng.meta.fetchone("SELECT config_json FROM connectors WHERE id=?", (cid,)))["config_json"])
        check("update: connector registered with objects config", any(o.get("match") == "*.secret" for o in before.get("objects", [])))

        await eng.add(root, config={"objects": [{"match": "*.py", "indexable": False}]}, update_config=True)
        cid2 = (await eng.meta.fetchone("SELECT id FROM connectors WHERE root_uri=?", (conn_uri,)))["id"]
        after = json.loads((await eng.meta.fetchone("SELECT config_json FROM connectors WHERE id=?", (cid2,)))["config_json"])
        check("K6 update: same connector (not re-registered)", cid2 == cid)
        check("K6 update: config_json reflects new objects rule",
