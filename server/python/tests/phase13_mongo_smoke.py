...
async def main():
    if not os.environ.get("OPENAI_API_KEY"):
        ...
        raise SystemExit(2)
    if not _mongo_up():
        print("mongo not reachable at 127.0.0.1:27017 — start the mfs-mongo container")
        raise SystemExit(2)
        ...
        [
            {
                ...
                "body": "Stripe webhook returns 402 intermittently",
                "status": "open",
            },
            {
                "ticket_id": 2,
                "title": "Login loop after SSO",
                "body": "Session token not persisted, user bounced to IdP",
                "status": "open",
            },
            {
                "ticket_id": 3,
                "title": "Export to CSV truncates",
                "body": "Large report cut at 65k rows",
                "status": "closed",
            },
        ]
    )

    base = f"/tmp/mfs_mongo_{os.getpid()}"
