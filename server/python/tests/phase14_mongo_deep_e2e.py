...
async def _seed(c: AsyncMongoClient) -> None:
    ...
    await db.tickets.insert_many([
        {"region": "us-east-1", "ticket_id": 100,
         "subject": "saml sso login loop",
         "body": "Identity provider redirect drops the session token",
         "priority": "high", "assignee": "alice",
         "payload": {"title": "infra", "body":
                     "cytochrome oxidase activity drops under hypoxia"},
         "updatedAt": "2026-05-01T10:00:00Z"},
        {"region": "us-east-1", "ticket_id": 101,
         "subject": "csv export truncated",
         "body": "Large report export is cut at 65000 rows",
         "priority": "medium", "assignee": "bob",
         "payload": {"title": "ops", "body":
                     "polymer chain entanglement governs the relaxation modulus"},
         "updatedAt": "2026-05-02T10:00:00Z"},
        {"region": "eu-west-1", "ticket_id": 100,
    ...
    await db.bigcol.insert_many([
        {"id": i, "body": f"doc {i} concerns topic-{i % 5}",
         "updatedAt": "2026-05-01T10:00:00Z"}
        for i in range(1, 51)])
...
async def main():
    ...
    try:
        ...
        print(f"mongo not reachable at {MONGO_URI}: {e}"); raise SystemExit(2)

    await _drop_db(client)
    await _seed(client)

    base = f"/tmp/mfs_mgdeep_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg_obj = {
        "uri": MONGO_URI, "database": DB_NAME,
        "objects": [
    ...
    try:
        ...
        want = {
            "/tickets/documents.jsonl", "/tickets/schema.json",
            "/audit/documents.jsonl", "/audit/schema.json",
            "/bigcol/documents.jsonl", "/bigcol/schema.json",
            "/empty/documents.jsonl", "/empty/schema.json",
        }
        check(f"T1 all 8 collection objects landed "
              f"(missing={sorted(want - set(uris))})",
              want <= set(uris))
        check(f"T1 audit collection independently indexed "
        ...
        check("T2 nested 'payload.body' lands in row_text "
        ...
        res_nest = await eng.search("cytochrome oxidase activity hypoxia",
                                     connector_uri=conn_uri, mode="hybrid", top_k=5)
        check(f"T2 semantic search hits the nested-field text "
              f"({len(res_nest)} hits)",
        ...
        sample = next(c for c in tk_chunks
                      if (c.get("locator") or {}).get("ticket_id") == 101
                      and (c.get("locator") or {}).get("region") == "us-east-1")
        loc = sample["locator"]
        check(f"T3 composite locator carries BOTH fields (got {loc})",
        ...
        check("T3 cat --locator composite key returns the right doc",
              recd.get("region") == "eu-west-1" and recd.get("ticket_id") == 100
        ...
        check(f"T4 priority metadata on every chunk (got {priorities})",
              priorities == {"high", "medium"})
        check(f"T4 assignee metadata on every chunk (got {assignees})",
              assignees == {"alice", "bob"})

        # ----- T5: chunk_max truncation -> 'partial' -----
        print("\n--- T5 · chunk_max truncation ---")
        big_ro = uris.get("/bigcol/documents.jsonl") or {}
        check(f"T5 chunk_max=10 caps chunk_count (got {big_ro.get('chunk_count')})",
              big_ro.get("chunk_count") == 10)
        check(f"T5 search_status='partial' (got {big_ro.get('search_status')!r})",
              big_ro.get("search_status") == "partial")
        big_hits = await eng.search("topic-2", connector_uri=conn_uri,
                                     object_prefix=conn_uri + "/bigcol",
                                     mode="hybrid", top_k=5)
        check(f"T5 'partial' slice still searchable ({len(big_hits)} hits)",
              len(big_hits) >= 1)

        # ----- T6: empty collection no-op -----
        print("\n--- T6 · empty collection no-op ---")
        empty_ro = uris.get("/empty/documents.jsonl") or {}
        check(f"T6 empty collection: chunks=0, no crash "
        ...
        check(f"T7 schema.json produces schema_summary "
        ...
        await client[DB_NAME].tickets.update_one(
            ...
            {"$set": {"body":
                "After identity provider redirect the session token is rotated.",
                "updatedAt": "2026-05-10T10:00:00Z"}})
        await eng.add(conn_uri, config=cfg_obj)
        res_mod = await eng.search("session token is rotated",
                                    connector_uri=conn_uri, mode="hybrid", top_k=5)
        check(f"T8 modify reflected in search ({len(res_mod)} hits)",
