...
class IncPlugin(ConnectorPlugin):
    ...
    NAME = "meminc"; URI_SCHEME = "meminc"; DISPLAY_NAME = "inc (test)"; PROMPT = "t"
    CAPABILITIES = Capabilities(manual_sync=True, delete_detection="state_change")

    def object_kind_of(self, path): return "document"
    async def stat(self, path): return PathStat(path=path, type="file", media_type="text/markdown")
    async def list(self, path): return []
    async def read(self, path, range=None) -> AsyncIterator[bytes]:
        yield b"# Note\nincremental content\n"
    async def fingerprint(self, path): return "v1"
...
class HCPlugin(ConnectorPlugin):
    NAME = "memhc"; URI_SCHEME = "memhc"; DISPLAY_NAME = "headcache (test)"; PROMPT = "t"
    CAPABILITIES = Capabilities(manual_sync=True, delete_detection="never", paged_cat=True)

    def object_kind_of(self, path): return "record_collection" if path.endswith("rows.jsonl") else "directory"
    async def stat(self, path): return PathStat(path=path, type="file", media_type="application/x-ndjson")
    async def list(self, path): return [Entry("rows.jsonl", "file")]
...
async def main():
    ...
    registry.register(IncPlugin); registry.register(HCPlugin)
    base = f"/tmp/mfs_he_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        kinds = await eng.meta.fetchall(
            "SELECT change_kind, count(*) AS n FROM object_tasks WHERE connector_id=? GROUP BY change_kind", (cid,))
        ...
        # B) head_cache
        hc_cfg = {"objects": [{"match": "*rows.jsonl", "text_fields": ["subject", "body"],
        ...
        art = await asyncio.to_thread(eng.object_store.get_artifact, eng.ns, "memhc://t/t/rows.jsonl", "head_cache")
