...
async def main():
    ...
    try:
        ...
        res = await eng.search("SSO login redirect fails for enterprise users",
                               connector_uri="postgres://test", mode="hybrid", top_k=3)
        check("search returns a ticket row", len(res) > 0 and any("rows.jsonl" in (e["source"] or "") for e in res))
        top = res[0] if res else {}
        check("hit chunk_kind=row_text", top.get("metadata", {}).get("chunk_kind") == "row_text")
        check("hit has locator with pk id", isinstance(top.get("locator"), dict) and "id" in (top.get("locator") or {}))
        check("top hit is the SSO ticket (id=1)", (top.get("locator") or {}).get("id") == 1)

        # keyword search on a distinct row
        res2 = await eng.search("dark theme toggle", connector_uri="postgres://test", mode="hybrid", top_k=3)
