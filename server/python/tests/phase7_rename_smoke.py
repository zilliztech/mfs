...
async def main():
    ...
    try:
        ...
        check(
            "rename: ZERO new embedding API calls (vectors reused)", eng.embed.api_calls == calls1
        )
        ...
        gone = await eng.meta.fetchone(
            "SELECT 1 FROM objects WHERE connector_id=? AND object_uri='/notes.md'", (conn["id"],)
        )
        newo = await eng.meta.fetchone(
            "SELECT chunk_count FROM objects WHERE connector_id=? AND object_uri='/networking.md'",
            (conn["id"],),
        )
        ...
        res = await eng.search(
            "tcp congestion retransmission", connector_uri=conn_uri, mode="hybrid", top_k=3
        )
        check(
            "rename: search hits new uri",
            any("networking.md" in (e["source"] or "") for e in res)
