...
async def main():
    ...
    da = tempfile.mkdtemp(prefix="mfs_ns_a_")
    db = tempfile.mkdtemp(prefix="mfs_ns_b_")
    ...
    eng_a = Engine(_mkcfg(base, "alpha"))
    await eng_a.startup()
    eng_b = Engine(_mkcfg(base, "beta"))
    await eng_b.startup()
    try:
        ca = eng_a.milvus.resolve_collection("alpha")
        cb = eng_b.milvus.resolve_collection("beta")
        ...
        check(
            "G4: ns 'alpha' cannot see ns 'beta' content",
            not any("b.md" in (e["source"] or "") for e in ra),
        )
        rb = await eng_b.search("quantum widget calibration", mode="hybrid", top_k=5)
        check(
            "G4: ns 'beta' cannot see ns 'alpha' content",
            not any("a.md" in (e["source"] or "") for e in rb),
        )
        ...
        d1 = tempfile.mkdtemp(prefix="mfs_g3_1_")
        d2 = tempfile.mkdtemp(prefix="mfs_g3_2_")
        same = "# Shared\n\nIdentical content used to prove the content-addressed cache.\n"
        open(f"{d1}/dup.md", "w").write(same)
        open(f"{d2}/dup.md", "w").write(same)
        await eng_a.add(d1)
        calls = eng_a.embed.api_calls
        await eng_a.add(d2)  # identical content -> cache hit
        check(
            "G3: identical-content second connector re-embeds nothing",
            eng_a.embed.api_calls == calls,
        )
