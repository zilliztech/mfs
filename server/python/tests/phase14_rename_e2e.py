...
async def _objs(eng: Engine, cid: str) -> dict[str, dict]:
    rows = await eng.meta.fetchall(
        ...
        "FROM objects WHERE connector_id=?",
        (cid,),
    )
...
async def _tasks(eng: Engine, cid: str) -> list[dict]:
    rows = await eng.meta.fetchall(
        ...
        "WHERE connector_id=? ORDER BY started_at DESC",
        (cid,),
    )
    return rows
...
async def main():
    ...
    base = f"/tmp/mfs_rename_{os.getpid()}"
    ...
    try:
        ...
        check(
            f"T1 initial sync embedded auth.py ({before_calls} api calls, "
        ...
        check(
            f"T1 sync yielded exactly one 'renamed' task ({len(renamed_t1)})", len(renamed_t1) == 1
        )
        check(
            "T1 renamed task points at new_uri AND old_uri",
            renamed_t1
            and renamed_t1[0]["object_uri"] == "/src/auth_v2.py"
            and renamed_t1[0]["old_uri"] == "/src/auth.py",
        )
        check(
            "T1 objects table: old path gone, new path present",
            "/src/auth.py" not in after_objs and "/src/auth_v2.py" in after_objs,
        )
        check(
            f"T1 chunk_count preserved across rename "
        ...
        check(
            f"T1 zero embedding API calls during rename (delta={eng.embed.api_calls})",
            eng.embed.api_calls == 0,
        )
        check(
            f"T1 Milvus has chunks at new uri, none at old "
        ...
        check(
            "T1 each new chunk pairs with an old chunk by line range",
            set(old_by_lines) == set(new_by_lines),
        )
        chunk_id_changed_all = all(
            old_by_lines[k]["chunk_id"] != new_by_lines[k]["chunk_id"] for k in old_by_lines
        )
        check("T1 chunk_id rewritten (new uri folded into the hash)", chunk_id_changed_all)
        # recompute the expected chunk_id and compare to what landed
        sample_lines = next(iter(new_by_lines))
        expected_id = chunk_id(
            ns, uri1, uri1 + "/src/auth_v2.py", "body", {"lines": list(sample_lines)}
        )
        check(
            "T1 new chunk_id matches sha1(ns + connector_uri + new_uri + ...)",
            expected_id in {c["chunk_id"] for c in new_chunks},
        )
        content_preserved = all(
            old_by_lines[k]["content"] == new_by_lines[k]["content"] for k in old_by_lines
        )
        ...
        check("T1 dense_vec preserved verbatim — vectors are reused, not recomputed", vec_preserved)
        ...
        check(
            "T2 yielded 'renamed' with /src/auth_v2.py -> /lib/auth_v2.py",
            len(renamed_t2) == 1 and renamed_t2[0]["old_uri"] == "/src/auth_v2.py",
        )
        check(
            "T2 old path removed, new path indexed",
            "/src/auth_v2.py" not in objs_t2 and "/lib/auth_v2.py" in objs_t2,
        )
        check(
            f"T2 zero embedding API calls (delta={eng.embed.api_calls})", eng.embed.api_calls == 0
        )
        chunks_t2 = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", uri1, uri1 + "/lib/auth_v2.py"
        )
        check(
            f"T2 Milvus chunks at the new path ({len(chunks_t2)})",
        ...
        check(
            f"T3 no 'renamed' task — sha1 differs and inodes differ (kinds={set(kinds)})",
            "renamed"
            not in {
                t["change_kind"]
                for t in tasks_t3
                if t["object_uri"] in ("/tools.py", "/helpers.py") or t["old_uri"] in ("/tools.py",)
            },
        )
        check("T3 old path removed", "/tools.py" not in objs_t3)
        check("T3 new path added", "/helpers.py" in objs_t3)
        check(
            f"T3 embedding API was called for the new content (delta={eng.embed.api_calls})",
        ...
        check(
            f"T4 sha1-fallback yielded 'renamed' "
            f"(/doc.md -> /doc_copy.md): {len(renamed_t4)} task(s)",
            len(renamed_t4) == 1 and renamed_t4[0]["old_uri"] == "/doc.md",
        )
        check(
            f"T4 vectors reused on sha1-pair (api delta={eng.embed.api_calls})",
            eng.embed.api_calls == 0,
        )
        ...
        check(
            f"T4 chunk count preserved ({len(old_chunks_t4)} -> {len(new_chunks_t4)})",
        ...
        check(f"T5 exactly two 'renamed' tasks ({len(renamed_t5)})", len(renamed_t5) == 2)
        check(
            "T5 both renamings pair up correctly (a->a2, b->b2)",
            ("/a.md", "/a2.md") in pairs and ("/b.md", "/b2.md") in pairs,
        )
        check(
            f"T5 zero embedding API calls (delta={eng.embed.api_calls})", eng.embed.api_calls == 0
        )
        objs_t5 = await _objs(eng, cid5)
        check(
            "T5 c.md untouched (no rename task, still indexed)",
            "/c.md" in objs_t5
            and not any(
                t["object_uri"] == "/c.md" and t["change_kind"] == "renamed" for t in tasks_t5
            ),
        )
        ...
        check(
            "T5 a's vectors reused under a2.md, b's under b2.md, c's unchanged",
            {tuple(c["dense_vec"]) for c in a_new} == {tuple(c["dense_vec"]) for c in a_old}
            and {tuple(c["dense_vec"]) for c in b_new} == {tuple(c["dense_vec"]) for c in b_old}
            and {tuple(c["dense_vec"]) for c in c_new} == {tuple(c["dense_vec"]) for c in c_old},
        )
        ...
        check(
            f"T6 pre-rename: converted_md artifact present (markitdown calls={first_md_calls})",
        ...
        await eng.add(str(repo6))
        check(
            f"T6 zero embedding API calls during rename (delta={eng.embed.api_calls})",
            eng.embed.api_calls == 0,
        )
        check(
            f"T6 zero markitdown calls during rename (delta={eng.converter.api_calls})",
            eng.converter.api_calls == 0,
        )
        ...
        old_dir = (
            pathlib.Path(cfg.object_store.root)
            / "artifacts"
            / ns
            / sha1_hex((uri6 + "/notes.html").encode())
        )
        new_dir = (
            pathlib.Path(cfg.object_store.root)
            / "artifacts"
            / ns
            / sha1_hex((uri6 + "/notes_v2.html").encode())
        )
        check(
            "T6 object_store: old artifact dir is gone after move_artifacts", not old_dir.exists()
        )
        check(
            "T6 object_store: new artifact dir holds the converted_md bytes",
            (new_dir / "converted_md").exists() and (new_dir / "converted_md").stat().st_size > 0,
        )
        ...
        check("T6 cat(new path) returns the cached markdown", "Octopus chromatophores" in body)
        check(
            f"T6 cat re-run added zero markitdown calls (delta={eng.converter.api_calls})",
            eng.converter.api_calls == 0,
        )
        ...
        row_at_old = await eng.meta.fetchone(
            ...
            "AND artifact_kind='converted_md'",
            (ns, uri6 + "/notes.html"),
        )
        ...
        check(
            "T6 artifact_cache row migrated to the new object_uri",
            row_at_old is None and row_at_new is not None,
        )
        check(
            "T6 artifact_cache.storage_path updated to the new per-uri dir",
        ...
        check(
            f"T6 cat(new uri) bumps last_accessed on the migrated row "
        ...
        t7_initial = [t for t in await _tasks(eng, cid7) if t["object_uri"] == "/src/foo.py"]
        check(
            f"T7 src/foo.py initial task priority = -220 (got "
        ...
        t7_after = [
            t
            for t in await _tasks(eng, cid7)
            if t["object_uri"] == "/tests/foo.py" and t["change_kind"] == "renamed"
        ]
        check(
            f"T7 renamed task priority follows new path = +80 (got "
        ...
        check(f"T8 initial embed cost ({first_calls_t8} call(s))", first_calls_t8 >= 1)
        ...
        check(
            "T8 only the final path /a3.md is indexed",
        ...
        check(
            f"T8 Milvus still holds the chunks at the final path ({len(chunks_t8)})",
            len(chunks_t8) >= 1,
        )
