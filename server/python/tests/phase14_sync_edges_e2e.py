...
async def main():
    ...
    base = f"/tmp/mfs_sync_{os.getpid()}"
    ...
    try:
        ...
        for o in objs_before:
            chunks_before += len(
                await asyncio.to_thread(eng.milvus.get_chunks_by_object, "default", uri1, uri1 + o)
            )
        check(
            f"T1 initial sync embedded {first_calls} batch call(s), "
        ...
        check(
            f"T1 --force-index re-enqueued every body task ({len(body_kinds)} "
            f"task rows present after force pass)",
            len(body_kinds) >= len(objs_before),
        )
        check(
            f"T1 --force-index: 0 embedding API calls (tx_cache absorbs cost) (delta={delta})",
            delta == 0,
        )
        ...
        check(
            f"T1 idempotent chunk_id upsert: Milvus chunk total unchanged "
        ...
        pre_fs = await eng.meta.fetchone(
            "SELECT 1 FROM file_state WHERE connector_id=? AND path='/notes.html'", (cid2,)
        )
        check(
            "T2 pre-delete: html object indexed, chunks present, artifact present, "
            "file_state row exists",
            "/notes.html" in pre_objects
            and len(pre_chunks) >= 1
            and pre_artifact_path is not None
            and os.path.exists(pre_artifact_path)
        ...
        check(f"T2 delete: Milvus chunks gone ({len(post_chunks)} left)", len(post_chunks) == 0)
        check("T2 delete: artifact_cache row gone", post_artifact_row is None)
        check(
            f"T2 delete: object_store bytes gone (path was {pre_artifact_path})",
        ...
        check(
            f"T2 second re-add after delete: no NEW task rows "
        ...
        job_row = await eng.meta.fetchone("SELECT status FROM connector_jobs WHERE id=?", (job_id,))
        check(
            f"T3 queued job after process=False (status={job_row['status']!r})",
        ...
        check(
            f"T3 tasks pending before worker run ({pending_before['n']})", pending_before["n"] >= 1
        )
        drained = await eng.run_worker_once()
        check(f"T3 run_worker_once drained the queued job ({drained!r})", drained == job_id)
        job_row2 = await eng.meta.fetchone(
            "SELECT status FROM connector_jobs WHERE id=?", (job_id,)
        )
        check(
            f"T3 job status='succeeded' after drain (got {job_row2['status']!r})",
            job_row2["status"] == "succeeded",
        )
        # objects now populated
        objs3 = await _objs(eng, cid3)
        check(f"T3 objects populated after worker run ({len(objs3)} rows)", len(objs3) >= 3)
        # idle worker call returns None
        none_drain = await eng.run_worker_once()
        check(
            f"T3 run_worker_once with empty queue returns None ({none_drain!r})", none_drain is None
        )
        ...
        try:
            ...
            check(
                f"T4 second add raised ValueError 'sync_already_running' ({str(e)!r})",
                "sync_already_running" in str(e),
            )
        ...
        check(
            f"T4 originally queued job eventually drained (status={post_first['status']!r})",
            post_first["status"] == "succeeded",
        )
        ...
        check(f"T5 first sync embed cost ({first_calls_t5} call(s))", first_calls_t5 >= 1)
        check(f"T5 second sync: 0 new tasks ({new_tasks})", new_tasks == 0)
        check(f"T5 second sync: 0 embed API calls (delta={delta_calls})", delta_calls == 0)
        ...
        post = await eng.meta.fetchone("SELECT status FROM connector_jobs WHERE id=?", (job6,))
        check(f"T6 job status='cancelled' (got {post['status']!r})", post["status"] == "cancelled")
        ...
        check(
            f"T6 no pending/running tasks left under the cancelled job ({pending_after['n']})",
            pending_after["n"] == 0,
        )
        # second cancel returns False (already terminal)
        again = await eng.cancel_job(job6)
        check(f"T6 cancel_job on already-cancelled job returns False ({again!r})", again is False)
        # the connector is unblocked: a fresh add succeeds
        await eng.add(str(repo6))
        objs6 = await _objs(eng, cid6)
        check(
            f"T6 connector unblocked after cancel: re-add indexed objects ({len(objs6)} rows)",
            len(objs6) >= 3,
        )
