...
async def main():
    ...
    eng._resolve_target = lambda t: (
        ("memcat", t, "memcat", {}) if t.startswith("memcat://") else orig(t)
    )
    await eng.startup()
    try:
        ...
        res = await eng.search(
            "refund invoice dispute", connector_uri="memcat://t", mode="hybrid", top_k=3
        )
        ...
        check(
            "cat --locator returns the exact record",
            rec.get("id") == 11 and "refund" in rec["body"],
        )
        ...
        check(
            "cat --range 0:2 -> first 2 records",
            len(lines) == 2 and json.loads(lines[0])["id"] == 10,
        )
        ...
        check(
            "ls lists rows.jsonl on structured connector",
