...
async def main():
    ...
    base = f"/tmp/mfs_hsdeep_{os.getpid()}"
    ...
    try:
        ...
        want = {"/contacts/records.jsonl", "/companies/records.jsonl", "/deals/records.jsonl"}
        check(
            f"T1 contacts + companies + deals all enumerate (missing={sorted(want - set(uris))})",
            want <= set(uris),
        )
        ...
        check(
            f"T2 contacts has >=1 chunk (got {contacts_ro.get('chunk_count')})",
            (contacts_ro.get("chunk_count") or 0) >= 1,
        )
        check(
            f"T2 contacts search_status='indexed' (got {contacts_ro.get('search_status')!r})",
        ...
        check(
            f"T3 companies has >=1 chunk (got {companies_ro.get('chunk_count')})",
        ...
        check(
            f"T4 empty deals: chunks=0, status='not_indexed' "
            f"(chunks={deals_ro.get('chunk_count')}, "
            f"status={deals_ro.get('search_status')!r})",
            deals_ro.get("chunk_count") == 0 and deals_ro.get("search_status") == "not_indexed",
        )
        ...
        check(
            f"T5 every contact chunk has a {{'id': ...}} locator (sample={ids[:2]})",
        ...
        check(
            f"T6 cached contact chunks contain known HubSpot tutorial "
            f"fixtures (Brian / hubspot.com)",
            "Brian" in joined or "hubspot.com" in joined,
        )
        hits = await eng.search("hubspot.com email", connector_uri=conn_uri, mode="hybrid", top_k=5)
        on_contacts = [h for h in hits if "/contacts/" in (h.get("source") or "")]
        check(
            f"T6 search lands on contacts subtree "
        ...
        if sample_id:
            ...
            check(
                f"T7 cat --locator {{'id': '{sample_id}'}} returns the record",
        ...
        check(
            f"T8 ls / lists the 3 configured objects (got {sorted(root_names)})",
        ...
        check(
            f"T8 ls /contacts contains records.jsonl (got {sorted(contacts_subdir)})",
        ...
        cap_cid = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri='hubspot://t14-capped'"
            )
        )["id"]
        ...
        check(
            f"T9 chunk_max=1 caps contacts (chunks={cap_ro['chunk_count'] if cap_ro else None})",
            cap_ro and cap_ro["chunk_count"] == 1,
        )
        check(
            f"T9 search_status='partial' on truncated contacts "
        ...
        row_only = await eng.search(
            "contact", connector_uri=conn_uri, mode="hybrid", top_k=5, chunk_kinds=["row_text"]
        )
        check(
            f"T10 chunk_kinds=['row_text'] gates retrieval ({len(row_only)} hits)",
            len(row_only) == 0
        ...
        check(
            f"T10 object_prefix='/contacts/' scopes search ({len(scoped)} hits)",
            len(scoped) == 0 or all("/contacts/" in (h.get("source") or "") for h in scoped),
        )
        ...
        check(
            f"T11 cat --range (0,2) returns 2 contact records (got {len(decoded)})",
            len(decoded) == 2,
        )
        check(
            "T11 each sliced record carries an id",
        ...
        kw_hits = await eng.search("hubspot", connector_uri=conn_uri, mode="keyword", top_k=5)
        sem_hits = await eng.search(
            "crm software vendor", connector_uri=conn_uri, mode="semantic", top_k=5
        )
        check(
            f"T12 keyword('hubspot') returns hit with literal term ({len(kw_hits)} hits)",
            any("hubspot" in (h.get("content") or "").lower() for h in kw_hits),
        )
        check(f"T12 semantic search returns hits ({len(sem_hits)} hits)", len(sem_hits) >= 1)
        ...
        check(
            f"T13 finding: plugin.fingerprint=None forces every object to "
            f"re-yield as modified ({delta_tasks} new tasks; expected >=3)",
            delta_tasks >= 3,
        )
        check(
            f"T13 second sync embeds 0 chunks — tx_cache absorbs the cost "
        ...
        check(
            f"T14 _DEFAULT_OBJECTS still ships with tickets "
        ...
        await eng.add("hubspot://t14-probe", config={"credential_ref": "env:HUBSPOT_ACCESS_TOKEN"})
        probe_cid = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri='hubspot://t14-probe'"
            )
        )["id"]
        ...
        check(
            f"T14 probe path: Free CRM enumerates contacts+companies+deals "
            f"(got {sorted(probe_paths)})",
            {"/contacts/records.jsonl", "/companies/records.jsonl", "/deals/records.jsonl"}
            <= probe_paths,
        )
        check(
            f"T14 probe path: tickets dropped — Service Hub not enabled "
            f"(got {sorted(probe_paths)})",
