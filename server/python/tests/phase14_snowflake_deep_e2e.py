...
async def main():
    ...
    if not os.path.exists(pk_file):
        print(f"private key not found: {pk_file}")
        raise SystemExit(2)
    ...
    await asyncio.to_thread(
        exec_, f"CREATE OR REPLACE TABLE {big_t} (ID INT PRIMARY KEY, BODY STRING)"
    )
    await asyncio.to_thread(
        cur.executemany,
        f"INSERT INTO {big_t} (ID, BODY) VALUES (%s, %s)",
        [(i, f"row {i} concerns topic-{i % 5} for snowflake indexing") for i in range(1, 31)],
    )

    # empty — 0 rows
    await asyncio.to_thread(
        exec_, f"CREATE OR REPLACE TABLE {empty_t} (ID INT PRIMARY KEY, BODY STRING)"
    )

    base = f"/tmp/mfs_sfd14_{os.getpid()}"
    ...
    try:
        ...
        for tname in (tickets_t, compkey_t, big_t, empty_t):
            ...
            check(f"T1 {tname}/rows.jsonl present", rows_path in uris)
        check(
            f"T1 tickets indexed (chunks={uris.get(f'/{db}/{sch}/tables/{tickets_t}/rows.jsonl', {}).get('chunk_count')})",
        ...
        check(
            "T2 cat --locator composite key reopens the right row",
            recd.get("REGION") == "eu-west-1"
            and recd.get("USER_ID") == 100
        ...
        check(
            f"T3 PRIORITY metadata present on every chunk (got {priorities})",
            priorities == {"high", "medium"},
        )
        check(
            f"T3 ASSIGNEE metadata present on every chunk (got {assignees})",
            assignees == {"alice", "bob"},
        )
        ...
        check(
            f"T4 chunk_max=8 caps chunk_count (got {big_ro.get('chunk_count')})",
            big_ro.get("chunk_count") == 8,
        )
        check(
            f"T4 search_status='partial' (got {big_ro.get('search_status')!r})",
            big_ro.get("search_status") == "partial",
        )
        big_uri = conn_uri + f"/{db}/{sch}/tables/{big_t}/rows.jsonl"
        partial_hits = await eng.search(
            "topic-3", connector_uri=conn_uri, object_prefix=big_uri, mode="hybrid", top_k=5
        )
        check(
            f"T4 'partial' slice is searchable ({len(partial_hits)} hits)", len(partial_hits) >= 1
        )
        ...
        check(
            f"T5 empty table: chunks=0, status='not_indexed', no crash "
        ...
        tickets_res = await eng.search(
            "saml identity provider", connector_uri=conn_uri, mode="hybrid", top_k=5
        )
        ck_res = await eng.search(
            "noisy neighbor shared volume", connector_uri=conn_uri, mode="hybrid", top_k=5
        )
        check(
            f"T6 search hit lands in tickets ({len(tickets_res)} hits)",
            any(tickets_t in (h.get("source") or "") for h in tickets_res),
        )
        check(
            f"T6 search hit lands in compkey ({len(ck_res)} hits)",
            any(compkey_t in (h.get("source") or "") for h in ck_res),
        )
        ...
        for t in (tickets_t, compkey_t, big_t, empty_t):
            try:
                await asyncio.to_thread(exec_, f"DROP TABLE IF EXISTS {db}.{sch}.{t}")
            except Exception:
                pass
        try:
