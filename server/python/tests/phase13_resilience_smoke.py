...
async def main():
    ...
    open(f"{root}/notes.md", "w").write("# Caching\n\nLRU eviction keeps the artifact store under budget.\n")
    base = f"/tmp/mfs_res_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    try:
        ...
        chunks = await asyncio.to_thread(eng.milvus.get_chunks_by_object, "default",
                                         f"file://local{root}", f"file://local{root}/notes.md")
        ...
        gone = await eng.meta.fetchone("SELECT 1 FROM connectors WHERE root_uri=?", (f"file://local{root}",))
