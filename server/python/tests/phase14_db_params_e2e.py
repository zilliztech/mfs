...
async def main():
    ...
    await cur.execute(f"CREATE TABLE `{big_tbl}` "
                      "(id INT PRIMARY KEY, body TEXT, kind VARCHAR(20))")
    rows = [(i, f"placeholder row {i} - {'odd' if i % 2 else 'even'}",
             "odd" if i % 2 else "even") for i in range(1, 51)]    # 50 rows
    await cur.executemany(
        f"INSERT INTO `{big_tbl}` (id, body, kind) VALUES (%s, %s, %s)", rows)

    await cur.execute(f"CREATE TABLE `{small_tbl}` "
                      "(id INT PRIMARY KEY, subject TEXT, priority VARCHAR(20))")
    await cur.executemany(
        f"INSERT INTO `{small_tbl}` (id, subject, priority) VALUES (%s, %s, %s)",
        [(1, "ssl certificate expiry alert", "high"),
         (2, "trivial typo on the about page", "low")])

    await cur.execute(f"CREATE TABLE `{notes_tbl}` "
                      "(note_id INT PRIMARY KEY, content TEXT, author VARCHAR(40))")
    await cur.executemany(
    ...
    await cur.execute(f"CREATE TABLE `{empty_tbl}` "
                      "(id INT PRIMARY KEY, body TEXT)")

    base = f"/tmp/mfs_dbparams_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg_a = {
        ...
        "objects": [{
            ...
            "metadata_fields": ["kind"], "chunk_max": 10,
        }],
    }
    ...
    cfg_b = {
        ...
        "objects": [
            {"match": f"/{small_tbl}/rows.jsonl",
             "text_fields": ["subject"], "metadata_fields": ["priority"],
             "locator_fields": ["id"]},
            {"match": f"/{notes_tbl}/rows.jsonl",
             "text_fields": ["content"], "metadata_fields": ["author"],
    ...
    try:
        ...
        cid = (await eng.meta.fetchone(
            "SELECT id FROM connectors WHERE root_uri=?", (conn_uriA,)))["id"]
        ro = await eng.meta.fetchone(
            "SELECT chunk_count, search_status FROM objects "
            "WHERE connector_id=? AND object_uri=?", (cid, f"/{big_tbl}/rows.jsonl"))
        check(f"chunk_max=10 caps chunk_count (got {ro['chunk_count']})",
              ro and ro["chunk_count"] == 10)
        check(f"chunk_max truncation flags search_status='partial' (got {ro['search_status']!r})",
              ro and ro["search_status"] == "partial")

        # === 2) multiple [[objects]] entries ===
        await eng.add(conn_uriB, config=cfg_b)
        cidB = (await eng.meta.fetchone(
            "SELECT id FROM connectors WHERE root_uri=?", (conn_uriB,)))["id"]
        ...
        check(f"multi-objects: small table indexed ({small_ro['chunk_count']} chunks)",
              small_ro and small_ro["chunk_count"] == 2)
        check(f"multi-objects: notes table indexed ({notes_ro['chunk_count']} chunks)",
              notes_ro and notes_ro["chunk_count"] == 2)
        # cat --locator on each table uses the table-specific locator_field
        small_recd_raw = await eng.cat(conn_uriB + f"/{small_tbl}/rows.jsonl",
                                        locator={"id": 1})
        import json as _json
        small_recd = _json.loads(small_recd_raw["content"])
        check("multi-objects: cat --locator on small table by 'id'",
              small_recd.get("id") == 1 and "ssl" in (small_recd.get("subject") or ""))
        notes_recd_raw = await eng.cat(conn_uriB + f"/{notes_tbl}/rows.jsonl",
                                        locator={"note_id": 101})
        notes_recd = _json.loads(notes_recd_raw["content"])
        check("multi-objects: cat --locator on notes table by 'note_id'",
        ...
        small_chunks = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uriB,
            conn_uriB + f"/{small_tbl}/rows.jsonl")
        priorities = {(c.get("metadata") or {}).get("priority") for c in small_chunks}
        check(f"metadata_fields=['priority'] attached to small chunks (got {priorities})",
              priorities == {"high", "low"})
        ...
        check(f"metadata_fields=['author'] attached to notes chunks (got {authors})",
              authors == {"alice", "bob"})

        # === 3) empty table ===
        await eng.add(conn_uriC, config=cfg_c)
        cidC = (await eng.meta.fetchone(
            "SELECT id FROM connectors WHERE root_uri=?", (conn_uriC,)))["id"]
        empty_ro = await eng.meta.fetchone(
            "SELECT chunk_count, search_status FROM objects "
            "WHERE connector_id=? AND object_uri=?", (cidC, f"/{empty_tbl}/rows.jsonl"))
        check(f"empty table: 0 chunks, no crash (chunks={empty_ro['chunk_count']}, "
