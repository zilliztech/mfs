...
async def main():
    ...
    base = f"/tmp/mfs_fsdeep_{os.getpid()}"
    ...
    try:
        ...
        doc_paths = [
            u for u in uris if u.startswith("/docs/") and u.endswith(".md") and DOC_TOKEN in u
        ]
        check(
            f"T1 connector registered + extra_docs doc indexed (doc_paths={doc_paths})",
            len(doc_paths) == 1,
        )
        doc_path = doc_paths[0]
        check(
            f"T1 doc body produced chunks (chunks={uris[doc_path]['chunk_count']})",
        ...
        check(
            f"T2 cat returns non-empty body (len={len(body)})",
            isinstance(body, str) and len(body.strip()) >= 200,
        )
        check(
            "T2 cat body is not the raw API payload "
            "(no raw 'block_id' / 'document_revision_id' JSON keys at top)",
            '"block_id"' not in body[:500] and '"document_revision_id"' not in body[:500],
        )
        ...
        if tokens:
            ...
            hits = await eng.search(term, connector_uri=conn_uri, mode="hybrid", top_k=5)
            on_doc = [h for h in hits if (h.get("source") or "") == conn_uri + doc_path]
            check(
                f"T3 search('{term[:20]}') surfaces the doc "
        ...
        body_only = await eng.search(
            "doc", connector_uri=conn_uri, mode="hybrid", top_k=10, chunk_kinds=["body"]
        )
        check(
            f"T4 every filtered hit is chunk_kind='body' ({len(body_only)} hits)",
            len(body_only) == 0
        ...
        check(
            f"T5 scoped hits never leave /docs/ ({len(scoped)} hits)",
            len(scoped) == 0 or all("/docs/" in (h.get("source") or "") for h in scoped),
        )
        ...
        check(f"T6 ls /docs includes the indexed doc (entries={len(names)})", len(names) >= 1)
        ...
        check(f"T7 second sync: 0 new body tasks ({new_tasks})", new_tasks == 0)
        check(
        ...
        check(
            f"T8 SDK domain for region='feishu' = FEISHU_DOMAIN",
            FeishuPlugin._sdk_domain("feishu") == FEISHU_DOMAIN,
        )
        check(
            f"T8 SDK domain for region='lark' = LARK_DOMAIN",
        ...
        if not oauth_path.is_file():
            check("T9 oauth.json absent (user-mode segment skipped, lenient pass)", True)
        else:
            ...
            check(
                f"T9 user-mode add succeeds, refresh_token rotated "
                f"(mtime {mtime_before} -> {mtime_after})",
                mtime_after >= mtime_before,
            )
            user_cid = (
                await eng.meta.fetchone("SELECT id FROM connectors WHERE root_uri=?", (user_conn,))
            )["id"]
            ...
            check(
                f"T9 user-mode also indexes the extra_doc ({len(user_docs)} doc rows)",
                len(user_docs) >= 1 and any((r["chunk_count"] or 0) >= 1 for r in user_docs),
            )
