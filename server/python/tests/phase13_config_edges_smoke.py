...
def _credential_edges():
    ...
    check("R7.1 present env resolves", Engine._resolve_ref("env:MFS_TEST_PRESENT_XYZ") == "topsecret")
    ...
    for scheme in ("secret:app/key", "vault:kv/data/app"):
        try:
            ...
            check(f"R7.2 {scheme.split(':')[0]}: rejected (not implemented)", "not implemented" in str(e))
    # missing file -> error
    try:
...
async def main():
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        ...
        base = f"/tmp/mfs_cfg_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
        ...
        try:
            ...
            sk = await eng.meta.fetchone(
                ...
                "WHERE c.root_uri=? AND o.object_uri='/skip.secret'", (conn_uri,))
            check("R7.4 indexable=false recorded but 0 chunks", sk is not None and sk["chunk_count"] == 0)
            res = await eng.search("metadata-only secret", connector_uri=conn_uri, mode="hybrid", top_k=5)
            check("R7.4 indexable=false not searchable", not any("skip.secret" in (e["source"] or "") for e in res))
            check("R7.4 sibling still indexed", any("keep.md" in (e["source"] or "")
                  for e in await eng.search("document indexed normally", connector_uri=conn_uri, mode="hybrid", top_k=5)))
        finally:
        ...
        base2 = f"/tmp/mfs_nocache_{os.getpid()}"; os.system(f"rm -rf '{base2}'*")
        cfg2 = _mkcfg(base2); cfg2.transformation_cache.enabled = False
        ...
        try:
            ...
            res = await eng2.search("vector search transformation cache", connector_uri=conn2, mode="hybrid", top_k=3)
            check("R8.1 cache-disabled: index+search works", len(res) > 0)
            calls = eng2.embed.api_calls
            await eng2.add(root2, full=True)
            check("R8.1 cache-disabled: force re-index re-embeds (no cache reuse)", eng2.embed.api_calls > calls)
        finally:
