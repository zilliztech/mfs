...
async def _dir_indexed_at(eng, conn_uri, relpath):
    rows = await asyncio.to_thread(
        eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + relpath
    )
    for r in rows:
...
async def main():
    ...
    try:
        ...
        check(
            "initial: all four dir summaries exist",
            all(x is not None for x in (a0, deep0, b0, root0)),
        )

        await asyncio.sleep(1.1)  # ensure a distinct ms timestamp
        open(f"{root}/a/deep/y.py", "w").write(
