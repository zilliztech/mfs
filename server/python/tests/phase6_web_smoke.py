...
async def main():
    ...
    try:
        ...
        job = await eng.add("web://example", config={
        ...
        check("crawled page indexed with chunks", page is not None and page["chunk_count"] > 0 and page["search_status"] == "indexed")

        res = await eng.search("domain for use in illustrative examples in documents",
                               connector_uri="web://example", mode="hybrid", top_k=3)
        check("search returns crawled page", len(res) > 0 and any("example.com" in (e["source"] or "") for e in res))

        # re-add: ETag 304 should skip unchanged page -> 0 new tasks (or unchanged)
        job2 = await eng.add("web://example")
        t2 = await eng.meta.fetchall("SELECT change_kind FROM object_tasks WHERE connector_job_id=?", (job2,))
        check("re-crawl: ETag/304 skips unchanged (0 tasks) or re-modified",
