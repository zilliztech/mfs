...
async def main():
    ...
    try:
        ...
        check(
            "README.md indexed",
            objs.get("/README.md", {}).get("chunk_count", 0) > 0
        ...
        res = await eng.search(
            "example repository for forking practice",
            connector_uri="github://spoon",
            mode="hybrid",
            top_k=3,
        )
        check(
            "search returns repo files",
            len(res) > 0 and any("README" in (e["source"] or "") for e in res[:3]),
        )

        # re-add: blob shas unchanged -> 0 tasks
        job2 = await eng.add("github://spoon")
        t2 = await eng.meta.fetchall(
            "SELECT id FROM object_tasks WHERE connector_job_id=?", (job2,)
        )
