...
async def main():
    ...
    if not os.environ.get("OPENAI_API_KEY") or not uri or not tok:
        print("need OPENAI_API_KEY + ZILLIZ_URI/_API_KEY")
        raise SystemExit(2)
    ns = f"cc{os.getpid()}"
    da = tempfile.mkdtemp(prefix="mfs_cc_a_")
    db = tempfile.mkdtemp(prefix="mfs_cc_b_")
    ...
    try:
        ...
        for jid, label in ((ja, "alpha"), (jb, "beta")):
            jr = await eng.meta.fetchone(
                "SELECT status, total_objects, succeeded_objects FROM connector_jobs WHERE id=?",
                (jid,),
            )
            check(
                f"F4: job {label} succeeded, total==succeeded",
                jr["status"] == "succeeded" and jr["total_objects"] == jr["succeeded_objects"],
            )

        ra = await eng.search(
            "raft leader election", connector_uri=f"file://local{da}", mode="hybrid", top_k=3
        )
        rb = await eng.search(
            "columnar zonemap pruning", connector_uri=f"file://local{db}", mode="hybrid", top_k=3
        )
        check(
            "F4: both connectors' data searchable in parallel-built index",
            any("a.md" in (e["source"] or "") for e in ra)
            and any("b.md" in (e["source"] or "") for e in rb),
        )
    finally:
        for d in (da, db):
            try:
                await eng.remove_connector(d)
            except Exception:
                pass
        try:
