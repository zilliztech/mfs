...
def _s3():
    import boto3
    return boto3.client("s3", endpoint_url=EP, aws_access_key_id="mfsadmin",
                        aws_secret_access_key="mfsadmin123", region_name="us-east-1")
...
async def main():
    ...
    try:
        s3 = _s3(); s3.list_buckets()
    except Exception as e:  # noqa: BLE001
        print(f"MinIO not reachable: {e}"); raise SystemExit(2)
    bucket = f"mfs-src-{os.getpid()}"
    try: s3.create_bucket(Bucket=bucket)
    except Exception: pass
    s3.put_object(Bucket=bucket, Key="docs/readme.md",
                  Body=b"# Platform\n\nThe ingestion pipeline overview and data flow.\n")
    s3.put_object(Bucket=bucket, Key="src/app.py",
                  Body=b"def charge(invoice):\n    # capture a payment for an invoice\n    return gateway.capture(invoice)\n")

    base = f"/tmp/mfs_s3src_{os.getpid()}"; os.system(f"rm -rf '{base}'*")
    ...
    cfg_obj = {"bucket": bucket, "endpoint_url": EP, "region": "us-east-1",
    ...
    try:
        ...
        objs = await eng.meta.fetchall("SELECT object_uri FROM objects WHERE connector_id=?", (cid,))
        uris = {o["object_uri"] for o in objs}
        check("s3 source indexed both keys", any("readme.md" in u for u in uris) and any("app.py" in u for u in uris))

        r1 = await eng.search("ingestion pipeline data flow", connector_uri=conn_uri, mode="hybrid", top_k=5)
        check("s3 source: md searchable", any("readme.md" in (e["source"] or "") for e in r1))
        r2 = await eng.search("capture a payment for an invoice", connector_uri=conn_uri, mode="hybrid", top_k=5)
