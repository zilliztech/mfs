...
async def main():
    ...
    base = f"/tmp/mfs_ghd14_{os.getpid()}"
    ...
    try:
        ...
        paths = sorted(uris)
        print(
            f"  DEBUG indexed {len(paths)} objects: {paths[:12]}{'...' if len(paths) > 12 else ''}"
        )
        ...
        readme_path = "/README"
        check(
            f"T1 README from default branch landed (uris contains /README? {readme_path in uris})",
            readme_path in uris,
        )
        check(
            f"T1 finding: README has no extension so it routes to binary "
        ...
        check(
            f"T2 issues.jsonl landed (chunks={uris.get(issues_path, {}).get('chunk_count')})",
            issues_path in uris and (uris[issues_path].get("chunk_count") or 0) >= 1,
        )
        check(
            f"T2 issues capped at max_read_rows=3 "
        ...
        check(
            f"T3 pulls.jsonl landed (chunks={uris.get(pulls_path, {}).get('chunk_count')})",
            pulls_path in uris and (uris[pulls_path].get("chunk_count") or 0) >= 1,
        )

        # ----- T4: at least one PR diff.patch indexed as document -----
        print("\n--- T4 · per-PR diff.patch as document ---")
        diff_uris = [u for u in uris if u.startswith("/_meta/pulls/") and u.endswith("/diff.patch")]
        check(f"T4 at least one diff.patch landed ({len(diff_uris)} patches)", len(diff_uris) >= 1)
        if diff_uris:
            diff_uri = diff_uris[0]
            check(
                f"T4 sample diff.patch produced chunks ({uris[diff_uri].get('chunk_count')})",
        ...
        iss_chunks = await asyncio.to_thread(
            eng.milvus.get_chunks_by_object, "default", conn_uri, conn_uri + issues_path
        )
        sample = next(
            (
                c
                for c in iss_chunks
                if isinstance(c.get("locator"), dict) and "number" in c["locator"]
            ),
            None,
        )
        check(
            f"T5 issue chunk carries {{number}} locator "
            f"(sample loc={sample.get('locator') if sample else None})",
            sample is not None,
        )
        if sample:
            issue_num = sample["locator"]["number"]
            cat_res = await eng.cat(conn_uri + issues_path, locator={"number": issue_num})
            recd = _json.loads(cat_res["content"])
            check(
                f"T5 cat --locator reopens issue #{issue_num} ",
                recd.get("number") == issue_num and ("title" in recd or "body" in recd),
            )

        # ----- T6: search hits both code AND meta -----
        print("\n--- T6 · search across the connector ---")
        # the canonical README literally says "Hello World!"
        readme_hits = await eng.search(
            "hello world", connector_uri=conn_uri, mode="hybrid", top_k=5
        )
        ...
        on_readme = any((h.get("source") or "").endswith("/README") for h in readme_hits)
        on_meta = any("_meta" in (h.get("source") or "") for h in readme_hits)
        check(
            f"T6 search hits land somewhere in the connector "
        ...
        body = (
            readme_body
            if isinstance(readme_body, str)
            else ((readme_body or {}).get("content") or "")
        )
        check(f"T7 cat README returns plain content ({body!r})", "Hello World" in body)
        ...
        cid2 = (
            await eng.meta.fetchone(
                "SELECT id FROM connectors WHERE root_uri=?", ("github://oct-test",)
            )
        )["id"]
        ...
        check(f"T8 'test' branch sync produces objects (count={len(uris2)})", len(uris2) >= 1)
